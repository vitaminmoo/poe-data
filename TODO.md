...
- [ ] Evaluate breaking cell validation logic in `src/dat_parser.rs` out into a separate modules, one per type, with another for arrays that can use the basic types as a second stage. The goal is to get `dat_parser.rs` to be primarily a parser.
