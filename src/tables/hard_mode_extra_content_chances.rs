...
pub static TABLE_HardModeExtraContentChances: LazyLock<Vec<HardModeExtraContentChancesRow>> = LazyLock::new(|| {
    let df = DAT_LOADER
        .write()
        .unwrap()
...
impl HardModeExtraContentChancesRef {
    ...
    pub fn iter() -> impl Iterator<Item = Self> {
        TABLE_HardModeExtraContentChances.iter().enumerate().map(|(i, _)| Self(i))
    }
    pub fn iter_with_refs() -> impl Iterator<Item = (Self, &'static HardModeExtraContentChancesRow)> {
        TABLE_HardModeExtraContentChances.iter().enumerate().map(|(i, x)| (Self(i), x))
    }
}
