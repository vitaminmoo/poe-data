...
pub static TABLE_CurrencyUseEffectsFromItem: LazyLock<Vec<CurrencyUseEffectsFromItemRow>> = LazyLock::new(|| {
    let df = DAT_LOADER
        .write()
        .unwrap()
...
impl CurrencyUseEffectsFromItemRef {
    ...
    pub fn iter() -> impl Iterator<Item = Self> {
        TABLE_CurrencyUseEffectsFromItem.iter().enumerate().map(|(i, _)| Self(i))
    }
    pub fn iter_with_refs() -> impl Iterator<Item = (Self, &'static CurrencyUseEffectsFromItemRow)> {
        TABLE_CurrencyUseEffectsFromItem.iter().enumerate().map(|(i, x)| (Self(i), x))
    }
}
