...
pub static TABLE_CosmeticsEquipPanelMode: LazyLock<Vec<CosmeticsEquipPanelModeRow>> = LazyLock::new(|| {
    let df = DAT_LOADER
        .write()
        .unwrap()
...
impl CosmeticsEquipPanelModeRef {
    ...
    pub fn iter() -> impl Iterator<Item = Self> {
        TABLE_CosmeticsEquipPanelMode.iter().enumerate().map(|(i, _)| Self(i))
    }
    pub fn iter_with_refs() -> impl Iterator<Item = (Self, &'static CosmeticsEquipPanelModeRow)> {
        TABLE_CosmeticsEquipPanelMode.iter().enumerate().map(|(i, x)| (Self(i), x))
    }
}
