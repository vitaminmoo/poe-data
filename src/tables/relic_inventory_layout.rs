...
pub static TABLE_RelicInventoryLayout: LazyLock<Vec<RelicInventoryLayoutRow>> = LazyLock::new(|| {
    let df = DAT_LOADER
        .write()
        .unwrap()
...
impl RelicInventoryLayoutRef {
    ...
    pub fn iter() -> impl Iterator<Item = Self> {
        TABLE_RelicInventoryLayout.iter().enumerate().map(|(i, _)| Self(i))
    }
    pub fn iter_with_refs() -> impl Iterator<Item = (Self, &'static RelicInventoryLayoutRow)> {
        TABLE_RelicInventoryLayout.iter().enumerate().map(|(i, x)| (Self(i), x))
    }
}
