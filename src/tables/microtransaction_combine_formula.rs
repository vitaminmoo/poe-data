...
pub static TABLE_MicrotransactionCombineFormula: LazyLock<Vec<MicrotransactionCombineFormulaRow>> =
    LazyLock::new(|| {
        let df = DAT_LOADER
            .write()
            .unwrap()
...
impl MicrotransactionCombineFormulaRef {
    ...
    pub fn iter() -> impl Iterator<Item = Self> {
        TABLE_MicrotransactionCombineFormula
            .iter()
            .enumerate()
            .map(|(i, _)| Self(i))
    }
    pub fn iter_with_refs(
    ) -> impl Iterator<Item = (Self, &'static MicrotransactionCombineFormulaRow)> {
        TABLE_MicrotransactionCombineFormula
            .iter()
            .enumerate()
            .map(|(i, x)| (Self(i), x))
