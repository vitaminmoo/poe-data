...
pub static TABLE_GeometryChannel: LazyLock<Vec<GeometryChannelRow>> = LazyLock::new(|| {
    let df = DAT_LOADER.write().unwrap().get_table("data/balance/geometrychannel.datc64").unwrap().clone();
    df.rows_iter()
...
impl GeometryChannelRef {
    ...
    pub fn iter() -> impl Iterator<Item = Self> {
        TABLE_GeometryChannel.iter().enumerate().map(|(i, _)| Self(i))
    }
    pub fn iter_with_refs() -> impl Iterator<Item = (Self, &'static GeometryChannelRow)> {
        TABLE_GeometryChannel.iter().enumerate().map(|(i, x)| (Self(i), x))
    }
}
