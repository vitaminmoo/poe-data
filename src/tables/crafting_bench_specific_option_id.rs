...
pub static TABLE_CraftingBenchSpecificOptionId: LazyLock<Vec<CraftingBenchSpecificOptionIdRow>> = LazyLock::new(|| {
    let df = DAT_LOADER
        .write()
        .unwrap()
...
impl CraftingBenchSpecificOptionIdRef {
    ...
    pub fn iter() -> impl Iterator<Item = Self> {
        TABLE_CraftingBenchSpecificOptionId.iter().enumerate().map(|(i, _)| Self(i))
    }
    pub fn iter_with_refs() -> impl Iterator<Item = (Self, &'static CraftingBenchSpecificOptionIdRow)> {
        TABLE_CraftingBenchSpecificOptionId.iter().enumerate().map(|(i, x)| (Self(i), x))
    }
}
