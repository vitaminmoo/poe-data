...
pub static TABLE_MicrotransactionChargeVariations: LazyLock<
    Vec<MicrotransactionChargeVariationsRow>,
> = LazyLock::new(|| {
    let df = DAT_LOADER
        .write()
        .unwrap()
...
impl MicrotransactionChargeVariationsRef {
    ...
    pub fn iter() -> impl Iterator<Item = Self> {
        TABLE_MicrotransactionChargeVariations
            .iter()
            .enumerate()
            .map(|(i, _)| Self(i))
    }
    pub fn iter_with_refs(
    ) -> impl Iterator<Item = (Self, &'static MicrotransactionChargeVariationsRow)> {
        TABLE_MicrotransactionChargeVariations
            .iter()
            .enumerate()
            .map(|(i, x)| (Self(i), x))
