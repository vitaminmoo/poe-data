...
pub static TABLE_StashtabAffinityByItemClassCategory: LazyLock<Vec<StashtabAffinityByItemClassCategoryRow>> = LazyLock::new(|| {
    let df = DAT_LOADER
        .write()
        .unwrap()
...
impl StashtabAffinityByItemClassCategoryRef {
    ...
    pub fn iter() -> impl Iterator<Item = Self> {
        TABLE_StashtabAffinityByItemClassCategory.iter().enumerate().map(|(i, _)| Self(i))
    }
    pub fn iter_with_refs() -> impl Iterator<Item = (Self, &'static StashtabAffinityByItemClassCategoryRow)> {
        TABLE_StashtabAffinityByItemClassCategory.iter().enumerate().map(|(i, x)| (Self(i), x))
    }
}
