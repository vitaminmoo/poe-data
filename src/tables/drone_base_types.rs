...
pub static TABLE_DroneBaseTypes: LazyLock<Vec<DroneBaseTypesRow>> = LazyLock::new(|| {
    let df = DAT_LOADER.write().unwrap().get_table("data/balance/dronebasetypes.datc64").unwrap().clone();
    df.rows_iter()
...
impl DroneBaseTypesRef {
    ...
    pub fn iter() -> impl Iterator<Item = Self> {
        TABLE_DroneBaseTypes.iter().enumerate().map(|(i, _)| Self(i))
    }
    pub fn iter_with_refs() -> impl Iterator<Item = (Self, &'static DroneBaseTypesRow)> {
        TABLE_DroneBaseTypes.iter().enumerate().map(|(i, x)| (Self(i), x))
    }
}
