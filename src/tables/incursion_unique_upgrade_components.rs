...
pub static TABLE_IncursionUniqueUpgradeComponents: LazyLock<
    Vec<IncursionUniqueUpgradeComponentsRow>,
> = LazyLock::new(|| {
    let df = DAT_LOADER
        .write()
        .unwrap()
...
impl IncursionUniqueUpgradeComponentsRef {
    ...
    pub fn iter() -> impl Iterator<Item = Self> {
        TABLE_IncursionUniqueUpgradeComponents
            .iter()
            .enumerate()
            .map(|(i, _)| Self(i))
    }
    pub fn iter_with_refs(
    ) -> impl Iterator<Item = (Self, &'static IncursionUniqueUpgradeComponentsRow)> {
        TABLE_IncursionUniqueUpgradeComponents
            .iter()
            .enumerate()
            .map(|(i, x)| (Self(i), x))
