...
pub static TABLE_EndgameMapLocation: LazyLock<Vec<EndgameMapLocationRow>> = LazyLock::new(|| {
    let df = DAT_LOADER.write().unwrap().get_table("data/balance/endgamemaplocation.datc64").unwrap().clone();
    df.rows_iter()
        .map(|row| EndgameMapLocationRow {
            ...
            r#biomes: {
                ...
                let values = df
                    ...
                    .collect::<Vec<i64>>();
                values.into_iter().map(|value| EndgameMapBiomesRef::new(value as usize)).collect()
            },
            r#adjacent_biomes: {
                // array_mutator column.array == true
...
impl EndgameMapLocationRef {
    ...
    pub fn iter() -> impl Iterator<Item = Self> {
        TABLE_EndgameMapLocation.iter().enumerate().map(|(i, _)| Self(i))
    }
    pub fn iter_with_refs() -> impl Iterator<Item = (Self, &'static EndgameMapLocationRow)> {
        TABLE_EndgameMapLocation.iter().enumerate().map(|(i, x)| (Self(i), x))
    }
}
