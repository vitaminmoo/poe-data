...
pub static TABLE_ExpeditionTerrainFeatures: LazyLock<Vec<ExpeditionTerrainFeaturesRow>> = LazyLock::new(|| {
    let df = DAT_LOADER
        .write()
        .unwrap()
        ...
        .map(|row| ExpeditionTerrainFeaturesRow {
            ...
            r#expedition_areas: {
                ...
                let values = df
                    ...
                    .collect::<Vec<i64>>();
                values.into_iter().map(|value| ExpeditionAreasRef::new(value as usize)).collect()
            },
            r#unknown84: {
                // array_mutator column.array == false && column.type != 'string|bool'
...
impl ExpeditionTerrainFeaturesRef {
    ...
    pub fn iter() -> impl Iterator<Item = Self> {
        TABLE_ExpeditionTerrainFeatures.iter().enumerate().map(|(i, _)| Self(i))
    }
    pub fn iter_with_refs() -> impl Iterator<Item = (Self, &'static ExpeditionTerrainFeaturesRow)> {
        TABLE_ExpeditionTerrainFeatures.iter().enumerate().map(|(i, x)| (Self(i), x))
    }
}
