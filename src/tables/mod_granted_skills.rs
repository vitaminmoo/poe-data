...
pub static TABLE_ModGrantedSkills: LazyLock<Vec<ModGrantedSkillsRow>> = LazyLock::new(|| {
    let df = DAT_LOADER.write().unwrap().get_table("data/balance/modgrantedskills.datc64").unwrap().clone();
    df.rows_iter()
...
impl ModGrantedSkillsRef {
    ...
    pub fn iter() -> impl Iterator<Item = Self> {
        TABLE_ModGrantedSkills.iter().enumerate().map(|(i, _)| Self(i))
    }
    pub fn iter_with_refs() -> impl Iterator<Item = (Self, &'static ModGrantedSkillsRow)> {
        TABLE_ModGrantedSkills.iter().enumerate().map(|(i, x)| (Self(i), x))
    }
}
