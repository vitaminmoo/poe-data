...
pub static TABLE_GraphicalItemReceptacleSlot: LazyLock<Vec<GraphicalItemReceptacleSlotRow>> = LazyLock::new(|| {
    let df = DAT_LOADER
        .write()
        .unwrap()
...
impl GraphicalItemReceptacleSlotRef {
    ...
    pub fn iter() -> impl Iterator<Item = Self> {
        TABLE_GraphicalItemReceptacleSlot.iter().enumerate().map(|(i, _)| Self(i))
    }
    pub fn iter_with_refs() -> impl Iterator<Item = (Self, &'static GraphicalItemReceptacleSlotRow)> {
        TABLE_GraphicalItemReceptacleSlot.iter().enumerate().map(|(i, x)| (Self(i), x))
    }
}
