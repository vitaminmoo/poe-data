...
pub static TABLE_AlternatePassiveAdditions: LazyLock<Vec<AlternatePassiveAdditionsRow>> = LazyLock::new(|| {
    let df = DAT_LOADER
        .write()
        .unwrap()
...
impl AlternatePassiveAdditionsRef {
    ...
    pub fn iter() -> impl Iterator<Item = Self> {
        TABLE_AlternatePassiveAdditions.iter().enumerate().map(|(i, _)| Self(i))
    }
    pub fn iter_with_refs() -> impl Iterator<Item = (Self, &'static AlternatePassiveAdditionsRow)> {
        TABLE_AlternatePassiveAdditions.iter().enumerate().map(|(i, x)| (Self(i), x))
    }
}
