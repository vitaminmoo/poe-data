...
pub static TABLE_BetrayalTargetLifeScalingPerLevel: LazyLock<Vec<BetrayalTargetLifeScalingPerLevelRow>> = LazyLock::new(|| {
    let df = DAT_LOADER
        .write()
        .unwrap()
...
impl BetrayalTargetLifeScalingPerLevelRef {
    ...
    pub fn iter() -> impl Iterator<Item = Self> {
        TABLE_BetrayalTargetLifeScalingPerLevel.iter().enumerate().map(|(i, _)| Self(i))
    }
    pub fn iter_with_refs() -> impl Iterator<Item = (Self, &'static BetrayalTargetLifeScalingPerLevelRow)> {
        TABLE_BetrayalTargetLifeScalingPerLevel.iter().enumerate().map(|(i, x)| (Self(i), x))
    }
}
