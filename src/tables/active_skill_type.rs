...
pub static TABLE_ActiveSkillType: LazyLock<Vec<ActiveSkillTypeRow>> = LazyLock::new(|| {
    let df = DAT_LOADER.write().unwrap().get_table("data/balance/activeskilltype.datc64").unwrap().clone();
    df.rows_iter()
...
impl ActiveSkillTypeRef {
    ...
    pub fn iter() -> impl Iterator<Item = Self> {
        TABLE_ActiveSkillType.iter().enumerate().map(|(i, _)| Self(i))
    }
    pub fn iter_with_refs() -> impl Iterator<Item = (Self, &'static ActiveSkillTypeRow)> {
        TABLE_ActiveSkillType.iter().enumerate().map(|(i, x)| (Self(i), x))
    }
}
