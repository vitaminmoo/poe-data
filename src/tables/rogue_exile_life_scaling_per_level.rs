...
pub struct RogueExileLifeScalingPerLevelRow {
    pub r#level: i32,
    pub r#additional_life: i32,
}

#[derive(Debug, Clone, Copy, Hash, PartialEq, Eq, Ord, PartialOrd)]
pub struct RogueExileLifeScalingPerLevelRef(pub usize);

impl Deref for RogueExileLifeScalingPerLevelRef {
    type Target = RogueExileLifeScalingPerLevelRow;
    fn deref(&self) -> &'static Self::Target {
        &TABLE_RogueExileLifeScalingPerLevel[self.0]
    }
}

impl RogueExileLifeScalingPerLevelRef {
    pub fn new(index: usize) -> Self {
        Self(index)
    }
    pub fn as_static_ref(&self) -> &'static RogueExileLifeScalingPerLevelRow {
        &TABLE_RogueExileLifeScalingPerLevel[self.0]
    }
    pub fn get(&self) -> &'static RogueExileLifeScalingPerLevelRow {
        &TABLE_RogueExileLifeScalingPerLevel[self.0]
    }
...
mod test {
    ...
    #[test]
    fn get_all_rows() {
        for row in TABLE_RogueExileLifeScalingPerLevel.iter() {
            black_box(row);
        }
    }
}
