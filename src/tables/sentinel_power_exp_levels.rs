...
pub static TABLE_SentinelPowerExpLevels: LazyLock<Vec<SentinelPowerExpLevelsRow>> = LazyLock::new(|| {
    let df = DAT_LOADER
        .write()
        .unwrap()
...
impl SentinelPowerExpLevelsRef {
    ...
    pub fn iter() -> impl Iterator<Item = Self> {
        TABLE_SentinelPowerExpLevels.iter().enumerate().map(|(i, _)| Self(i))
    }
    pub fn iter_with_refs() -> impl Iterator<Item = (Self, &'static SentinelPowerExpLevelsRow)> {
        TABLE_SentinelPowerExpLevels.iter().enumerate().map(|(i, x)| (Self(i), x))
    }
}
