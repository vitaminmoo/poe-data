...
pub static TABLE_AfflictionRewardTypeVisuals: LazyLock<Vec<AfflictionRewardTypeVisualsRow>> = LazyLock::new(|| {
    let df = DAT_LOADER
        .write()
        .unwrap()
...
impl AfflictionRewardTypeVisualsRef {
    ...
    pub fn iter() -> impl Iterator<Item = Self> {
        TABLE_AfflictionRewardTypeVisuals.iter().enumerate().map(|(i, _)| Self(i))
    }
    pub fn iter_with_refs() -> impl Iterator<Item = (Self, &'static AfflictionRewardTypeVisualsRow)> {
        TABLE_AfflictionRewardTypeVisuals.iter().enumerate().map(|(i, x)| (Self(i), x))
    }
}
