...
pub static TABLE_MicrotransactionEquippedIconVariations: LazyLock<Vec<MicrotransactionEquippedIconVariationsRow>> = LazyLock::new(|| {
    let df = DAT_LOADER
        .write()
        .unwrap()
...
impl MicrotransactionEquippedIconVariationsRef {
    ...
    pub fn iter() -> impl Iterator<Item = Self> {
        TABLE_MicrotransactionEquippedIconVariations.iter().enumerate().map(|(i, _)| Self(i))
    }
    pub fn iter_with_refs() -> impl Iterator<Item = (Self, &'static MicrotransactionEquippedIconVariationsRow)> {
        TABLE_MicrotransactionEquippedIconVariations.iter().enumerate().map(|(i, x)| (Self(i), x))
    }
}
