...
pub struct MinionStatsRef(pub usize);

impl Deref for MinionStatsRef {
    type Target = MinionStatsRow;
    fn deref(&self) -> &'static Self::Target {
        &TABLE_MinionStats[self.0]
    }
}
...
impl MinionStatsRef {
    ...
    pub fn as_static_ref(&self) -> &'static MinionStatsRow {
        &TABLE_MinionStats[self.0]
    }
    pub fn get(&self) -> &'static MinionStatsRow {
        &TABLE_MinionStats[self.0]
    }
...
mod test {
    ...
    #[test]
    fn get_all_rows() {
        for row in TABLE_MinionStats.iter() {
            black_box(row);
        }
    }
}
