...
pub static TABLE_LeagueInfoPanelVersions: LazyLock<Vec<LeagueInfoPanelVersionsRow>> = LazyLock::new(|| {
    let df = DAT_LOADER
        .write()
        .unwrap()
...
impl LeagueInfoPanelVersionsRef {
    ...
    pub fn iter() -> impl Iterator<Item = Self> {
        TABLE_LeagueInfoPanelVersions.iter().enumerate().map(|(i, _)| Self(i))
    }
    pub fn iter_with_refs() -> impl Iterator<Item = (Self, &'static LeagueInfoPanelVersionsRow)> {
        TABLE_LeagueInfoPanelVersions.iter().enumerate().map(|(i, x)| (Self(i), x))
    }
}
