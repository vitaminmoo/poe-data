...
pub static TABLE_UniqueJewelLimits: LazyLock<Vec<UniqueJewelLimitsRow>> = LazyLock::new(|| {
    let df = DAT_LOADER.write().unwrap().get_table("data/balance/uniquejewellimits.datc64").unwrap().clone();
    df.rows_iter()
...
impl UniqueJewelLimitsRef {
    ...
    pub fn iter() -> impl Iterator<Item = Self> {
        TABLE_UniqueJewelLimits.iter().enumerate().map(|(i, _)| Self(i))
    }
    pub fn iter_with_refs() -> impl Iterator<Item = (Self, &'static UniqueJewelLimitsRow)> {
        TABLE_UniqueJewelLimits.iter().enumerate().map(|(i, x)| (Self(i), x))
    }
}
