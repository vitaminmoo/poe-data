...
pub static TABLE_HideoutDoodadCategory: LazyLock<Vec<HideoutDoodadCategoryRow>> = LazyLock::new(|| {
    let df = DAT_LOADER
        .write()
        .unwrap()
...
impl HideoutDoodadCategoryRef {
    ...
    pub fn iter() -> impl Iterator<Item = Self> {
        TABLE_HideoutDoodadCategory.iter().enumerate().map(|(i, _)| Self(i))
    }
    pub fn iter_with_refs() -> impl Iterator<Item = (Self, &'static HideoutDoodadCategoryRow)> {
        TABLE_HideoutDoodadCategory.iter().enumerate().map(|(i, x)| (Self(i), x))
    }
}
