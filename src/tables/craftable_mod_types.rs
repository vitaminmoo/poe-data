...
pub static TABLE_CraftableModTypes: LazyLock<Vec<CraftableModTypesRow>> = LazyLock::new(|| {
    let df = DAT_LOADER.write().unwrap().get_table("data/balance/craftablemodtypes.datc64").unwrap().clone();
    df.rows_iter()
...
impl CraftableModTypesRef {
    ...
    pub fn iter() -> impl Iterator<Item = Self> {
        TABLE_CraftableModTypes.iter().enumerate().map(|(i, _)| Self(i))
    }
    pub fn iter_with_refs() -> impl Iterator<Item = (Self, &'static CraftableModTypesRow)> {
        TABLE_CraftableModTypes.iter().enumerate().map(|(i, x)| (Self(i), x))
    }
}
