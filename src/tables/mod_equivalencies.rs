...
pub static TABLE_ModEquivalencies: LazyLock<Vec<ModEquivalenciesRow>> = LazyLock::new(|| {
    let df = DAT_LOADER.write().unwrap().get_table("data/balance/modequivalencies.datc64").unwrap().clone();
    df.rows_iter()
...
impl ModEquivalenciesRef {
    ...
    pub fn iter() -> impl Iterator<Item = Self> {
        TABLE_ModEquivalencies.iter().enumerate().map(|(i, _)| Self(i))
    }
    pub fn iter_with_refs() -> impl Iterator<Item = (Self, &'static ModEquivalenciesRow)> {
        TABLE_ModEquivalencies.iter().enumerate().map(|(i, x)| (Self(i), x))
    }
}
