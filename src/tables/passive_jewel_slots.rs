...
pub static TABLE_PassiveJewelSlots: LazyLock<Vec<PassiveJewelSlotsRow>> = LazyLock::new(|| {
    let df = DAT_LOADER.write().unwrap().get_table("data/balance/passivejewelslots.datc64").unwrap().clone();
    df.rows_iter()
...
impl PassiveJewelSlotsRef {
    ...
    pub fn iter() -> impl Iterator<Item = Self> {
        TABLE_PassiveJewelSlots.iter().enumerate().map(|(i, _)| Self(i))
    }
    pub fn iter_with_refs() -> impl Iterator<Item = (Self, &'static PassiveJewelSlotsRow)> {
        TABLE_PassiveJewelSlots.iter().enumerate().map(|(i, x)| (Self(i), x))
    }
}
