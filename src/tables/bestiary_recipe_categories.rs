...
pub static TABLE_BestiaryRecipeCategories: LazyLock<Vec<BestiaryRecipeCategoriesRow>> = LazyLock::new(|| {
    let df = DAT_LOADER
        .write()
        .unwrap()
...
impl BestiaryRecipeCategoriesRef {
    ...
    pub fn iter() -> impl Iterator<Item = Self> {
        TABLE_BestiaryRecipeCategories.iter().enumerate().map(|(i, _)| Self(i))
    }
    pub fn iter_with_refs() -> impl Iterator<Item = (Self, &'static BestiaryRecipeCategoriesRow)> {
        TABLE_BestiaryRecipeCategories.iter().enumerate().map(|(i, x)| (Self(i), x))
    }
}
