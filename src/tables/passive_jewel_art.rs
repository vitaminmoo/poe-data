...
pub static TABLE_PassiveJewelArt: LazyLock<Vec<PassiveJewelArtRow>> = LazyLock::new(|| {
    let df = DAT_LOADER.write().unwrap().get_table("data/balance/passivejewelart.datc64").unwrap().clone();
    df.rows_iter()
...
impl PassiveJewelArtRef {
    ...
    pub fn iter() -> impl Iterator<Item = Self> {
        TABLE_PassiveJewelArt.iter().enumerate().map(|(i, _)| Self(i))
    }
    pub fn iter_with_refs() -> impl Iterator<Item = (Self, &'static PassiveJewelArtRow)> {
        TABLE_PassiveJewelArt.iter().enumerate().map(|(i, x)| (Self(i), x))
    }
}
