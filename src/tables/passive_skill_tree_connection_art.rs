...
pub static TABLE_PassiveSkillTreeConnectionArt: LazyLock<Vec<PassiveSkillTreeConnectionArtRow>> =
    LazyLock::new(|| {
        let df = DAT_LOADER
            .write()
            .unwrap()
...
impl PassiveSkillTreeConnectionArtRef {
    ...
    pub fn iter() -> impl Iterator<Item = Self> {
        TABLE_PassiveSkillTreeConnectionArt
            .iter()
            .enumerate()
            .map(|(i, _)| Self(i))
    }
    pub fn iter_with_refs(
    ) -> impl Iterator<Item = (Self, &'static PassiveSkillTreeConnectionArtRow)> {
        TABLE_PassiveSkillTreeConnectionArt
            .iter()
            .enumerate()
            .map(|(i, x)| (Self(i), x))
