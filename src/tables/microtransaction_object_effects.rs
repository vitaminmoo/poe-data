...
pub static TABLE_MicrotransactionObjectEffects: LazyLock<Vec<MicrotransactionObjectEffectsRow>> = LazyLock::new(|| {
    let df = DAT_LOADER
        .write()
        .unwrap()
...
impl MicrotransactionObjectEffectsRef {
    ...
    pub fn iter() -> impl Iterator<Item = Self> {
        TABLE_MicrotransactionObjectEffects.iter().enumerate().map(|(i, _)| Self(i))
    }
    pub fn iter_with_refs() -> impl Iterator<Item = (Self, &'static MicrotransactionObjectEffectsRow)> {
        TABLE_MicrotransactionObjectEffects.iter().enumerate().map(|(i, x)| (Self(i), x))
    }
}
