...
pub static TABLE_SkillSurgeEffects: LazyLock<Vec<SkillSurgeEffectsRow>> = LazyLock::new(|| {
    let df = DAT_LOADER.write().unwrap().get_table("data/balance/skillsurgeeffects.datc64").unwrap().clone();
    df.rows_iter()
...
impl SkillSurgeEffectsRef {
    ...
    pub fn iter() -> impl Iterator<Item = Self> {
        TABLE_SkillSurgeEffects.iter().enumerate().map(|(i, _)| Self(i))
    }
    pub fn iter_with_refs() -> impl Iterator<Item = (Self, &'static SkillSurgeEffectsRow)> {
        TABLE_SkillSurgeEffects.iter().enumerate().map(|(i, x)| (Self(i), x))
    }
}
