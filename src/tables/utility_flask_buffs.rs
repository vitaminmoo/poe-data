...
pub static TABLE_UtilityFlaskBuffs: LazyLock<Vec<UtilityFlaskBuffsRow>> = LazyLock::new(|| {
    let df = DAT_LOADER.write().unwrap().get_table("data/balance/utilityflaskbuffs.datc64").unwrap().clone();
    df.rows_iter()
...
impl UtilityFlaskBuffsRef {
    ...
    pub fn iter() -> impl Iterator<Item = Self> {
        TABLE_UtilityFlaskBuffs.iter().enumerate().map(|(i, _)| Self(i))
    }
    pub fn iter_with_refs() -> impl Iterator<Item = (Self, &'static UtilityFlaskBuffsRow)> {
        TABLE_UtilityFlaskBuffs.iter().enumerate().map(|(i, x)| (Self(i), x))
    }
}
