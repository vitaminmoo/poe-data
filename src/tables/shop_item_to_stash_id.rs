...
pub static TABLE_ShopItemToStashId: LazyLock<Vec<ShopItemToStashIdRow>> = LazyLock::new(|| {
    let df = DAT_LOADER.write().unwrap().get_table("data/balance/shopitemtostashid.datc64").unwrap().clone();
    df.rows_iter()
...
impl ShopItemToStashIdRef {
    ...
    pub fn iter() -> impl Iterator<Item = Self> {
        TABLE_ShopItemToStashId.iter().enumerate().map(|(i, _)| Self(i))
    }
    pub fn iter_with_refs() -> impl Iterator<Item = (Self, &'static ShopItemToStashIdRow)> {
        TABLE_ShopItemToStashId.iter().enumerate().map(|(i, x)| (Self(i), x))
    }
}
