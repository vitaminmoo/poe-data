...
pub static TABLE_MiscBeamsArtVariations: LazyLock<Vec<MiscBeamsArtVariationsRow>> = LazyLock::new(|| {
    let df = DAT_LOADER
        .write()
        .unwrap()
...
impl MiscBeamsArtVariationsRef {
    ...
    pub fn iter() -> impl Iterator<Item = Self> {
        TABLE_MiscBeamsArtVariations.iter().enumerate().map(|(i, _)| Self(i))
    }
    pub fn iter_with_refs() -> impl Iterator<Item = (Self, &'static MiscBeamsArtVariationsRow)> {
        TABLE_MiscBeamsArtVariations.iter().enumerate().map(|(i, x)| (Self(i), x))
    }
}
