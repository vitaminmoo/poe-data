...
pub static TABLE_SkillGemLevelUpEffects: LazyLock<Vec<SkillGemLevelUpEffectsRow>> = LazyLock::new(|| {
    let df = DAT_LOADER
        .write()
        .unwrap()
...
impl SkillGemLevelUpEffectsRef {
    ...
    pub fn iter() -> impl Iterator<Item = Self> {
        TABLE_SkillGemLevelUpEffects.iter().enumerate().map(|(i, _)| Self(i))
    }
    pub fn iter_with_refs() -> impl Iterator<Item = (Self, &'static SkillGemLevelUpEffectsRow)> {
        TABLE_SkillGemLevelUpEffects.iter().enumerate().map(|(i, x)| (Self(i), x))
    }
}
