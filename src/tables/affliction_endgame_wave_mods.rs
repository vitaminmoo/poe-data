...
pub static TABLE_AfflictionEndgameWaveMods: LazyLock<Vec<AfflictionEndgameWaveModsRow>> = LazyLock::new(|| {
    let df = DAT_LOADER
        .write()
        .unwrap()
...
impl AfflictionEndgameWaveModsRef {
    ...
    pub fn iter() -> impl Iterator<Item = Self> {
        TABLE_AfflictionEndgameWaveMods.iter().enumerate().map(|(i, _)| Self(i))
    }
    pub fn iter_with_refs() -> impl Iterator<Item = (Self, &'static AfflictionEndgameWaveModsRow)> {
        TABLE_AfflictionEndgameWaveMods.iter().enumerate().map(|(i, x)| (Self(i), x))
    }
}
