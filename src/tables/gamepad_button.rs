...
pub struct GamepadButtonRef(pub usize);

impl Deref for GamepadButtonRef {
    type Target = GamepadButtonRow;
    fn deref(&self) -> &'static Self::Target {
        &TABLE_GamepadButton[self.0]
    }
}

impl GamepadButtonRef {
    pub fn new(index: usize) -> Self {
        Self(index)
    }
    pub fn as_static_ref(&self) -> &'static GamepadButtonRow {
        &TABLE_GamepadButton[self.0]
    }
    pub fn get(&self) -> &'static GamepadButtonRow {
        &TABLE_GamepadButton[self.0]
    }
...
mod test {
    ...
    #[test]
    fn get_all_rows() {
        for row in TABLE_GamepadButton.iter() {
            black_box(row);
        }
    }
}
