...
pub static TABLE_HideoutWaypointDoodads: LazyLock<Vec<HideoutWaypointDoodadsRow>> = LazyLock::new(|| {
    let df = DAT_LOADER
        .write()
        .unwrap()
...
impl HideoutWaypointDoodadsRef {
    ...
    pub fn iter() -> impl Iterator<Item = Self> {
        TABLE_HideoutWaypointDoodads.iter().enumerate().map(|(i, _)| Self(i))
    }
    pub fn iter_with_refs() -> impl Iterator<Item = (Self, &'static HideoutWaypointDoodadsRow)> {
        TABLE_HideoutWaypointDoodads.iter().enumerate().map(|(i, x)| (Self(i), x))
    }
}
