...
pub static TABLE_HudEnergyShieldVisuals: LazyLock<Vec<HudEnergyShieldVisualsRow>> = LazyLock::new(|| {
    let df = DAT_LOADER
        .write()
        .unwrap()
...
impl HudEnergyShieldVisualsRef {
    ...
    pub fn iter() -> impl Iterator<Item = Self> {
        TABLE_HudEnergyShieldVisuals.iter().enumerate().map(|(i, _)| Self(i))
    }
    pub fn iter_with_refs() -> impl Iterator<Item = (Self, &'static HudEnergyShieldVisualsRow)> {
        TABLE_HudEnergyShieldVisuals.iter().enumerate().map(|(i, x)| (Self(i), x))
    }
}
