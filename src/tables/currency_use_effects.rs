...
pub static TABLE_CurrencyUseEffects: LazyLock<Vec<CurrencyUseEffectsRow>> = LazyLock::new(|| {
    let df = DAT_LOADER.write().unwrap().get_table("data/balance/currencyuseeffects.datc64").unwrap().clone();
    df.rows_iter()
...
impl CurrencyUseEffectsRef {
    ...
    pub fn iter() -> impl Iterator<Item = Self> {
        TABLE_CurrencyUseEffects.iter().enumerate().map(|(i, _)| Self(i))
    }
    pub fn iter_with_refs() -> impl Iterator<Item = (Self, &'static CurrencyUseEffectsRow)> {
        TABLE_CurrencyUseEffects.iter().enumerate().map(|(i, x)| (Self(i), x))
    }
}
