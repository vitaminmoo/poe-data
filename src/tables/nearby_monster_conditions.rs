...
pub static TABLE_NearbyMonsterConditions: LazyLock<Vec<NearbyMonsterConditionsRow>> = LazyLock::new(|| {
    let df = DAT_LOADER
        .write()
        .unwrap()
...
impl NearbyMonsterConditionsRef {
    ...
    pub fn iter() -> impl Iterator<Item = Self> {
        TABLE_NearbyMonsterConditions.iter().enumerate().map(|(i, _)| Self(i))
    }
    pub fn iter_with_refs() -> impl Iterator<Item = (Self, &'static NearbyMonsterConditionsRow)> {
        TABLE_NearbyMonsterConditions.iter().enumerate().map(|(i, x)| (Self(i), x))
    }
}
