...
pub static TABLE_ExpeditionCurrency: LazyLock<Vec<ExpeditionCurrencyRow>> = LazyLock::new(|| {
    let df = DAT_LOADER.write().unwrap().get_table("data/balance/expeditioncurrency.datc64").unwrap().clone();
    df.rows_iter()
...
impl ExpeditionCurrencyRef {
    ...
    pub fn iter() -> impl Iterator<Item = Self> {
        TABLE_ExpeditionCurrency.iter().enumerate().map(|(i, _)| Self(i))
    }
    pub fn iter_with_refs() -> impl Iterator<Item = (Self, &'static ExpeditionCurrencyRow)> {
        TABLE_ExpeditionCurrency.iter().enumerate().map(|(i, x)| (Self(i), x))
    }
}
