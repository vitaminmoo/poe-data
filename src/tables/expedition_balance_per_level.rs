...
pub static TABLE_ExpeditionBalancePerLevel: LazyLock<Vec<ExpeditionBalancePerLevelRow>> = LazyLock::new(|| {
    let df = DAT_LOADER
        .write()
        .unwrap()
...
impl ExpeditionBalancePerLevelRef {
    ...
    pub fn iter() -> impl Iterator<Item = Self> {
        TABLE_ExpeditionBalancePerLevel.iter().enumerate().map(|(i, _)| Self(i))
    }
    pub fn iter_with_refs() -> impl Iterator<Item = (Self, &'static ExpeditionBalancePerLevelRow)> {
        TABLE_ExpeditionBalancePerLevel.iter().enumerate().map(|(i, x)| (Self(i), x))
    }
}
