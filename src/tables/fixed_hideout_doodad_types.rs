...
pub static TABLE_FixedHideoutDoodadTypes: LazyLock<Vec<FixedHideoutDoodadTypesRow>> = LazyLock::new(|| {
    let df = DAT_LOADER
        .write()
        .unwrap()
        ...
        .map(|row| FixedHideoutDoodadTypesRow {
            ...
            r#hideout_doodads: {
                ...
                let values = df
                    ...
                    .collect::<Vec<i64>>();
                values.into_iter().map(|value| HideoutDoodadsRef::new(value as usize)).collect()
            },
...
impl FixedHideoutDoodadTypesRef {
    ...
    pub fn iter() -> impl Iterator<Item = Self> {
        TABLE_FixedHideoutDoodadTypes.iter().enumerate().map(|(i, _)| Self(i))
    }
    pub fn iter_with_refs() -> impl Iterator<Item = (Self, &'static FixedHideoutDoodadTypesRow)> {
        TABLE_FixedHideoutDoodadTypes.iter().enumerate().map(|(i, x)| (Self(i), x))
    }
}
