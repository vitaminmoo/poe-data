...
pub static TABLE_MonsterMapBossDifficulty: LazyLock<Vec<MonsterMapBossDifficultyRow>> = LazyLock::new(|| {
    let df = DAT_LOADER
        .write()
        .unwrap()
...
impl MonsterMapBossDifficultyRef {
    ...
    pub fn iter() -> impl Iterator<Item = Self> {
        TABLE_MonsterMapBossDifficulty.iter().enumerate().map(|(i, _)| Self(i))
    }
    pub fn iter_with_refs() -> impl Iterator<Item = (Self, &'static MonsterMapBossDifficultyRow)> {
        TABLE_MonsterMapBossDifficulty.iter().enumerate().map(|(i, x)| (Self(i), x))
    }
}
