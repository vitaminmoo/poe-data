...
pub static TABLE_HarvestSeedItems: LazyLock<Vec<HarvestSeedItemsRow>> = LazyLock::new(|| {
    let df = DAT_LOADER.write().unwrap().get_table("data/balance/harvestseeditems.datc64").unwrap().clone();
    df.rows_iter()
...
impl HarvestSeedItemsRef {
    ...
    pub fn iter() -> impl Iterator<Item = Self> {
        TABLE_HarvestSeedItems.iter().enumerate().map(|(i, _)| Self(i))
    }
    pub fn iter_with_refs() -> impl Iterator<Item = (Self, &'static HarvestSeedItemsRow)> {
        TABLE_HarvestSeedItems.iter().enumerate().map(|(i, x)| (Self(i), x))
    }
}
