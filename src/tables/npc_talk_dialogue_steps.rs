...
pub static TABLE_NPCTalkDialogueSteps: LazyLock<Vec<NPCTalkDialogueStepsRow>> = LazyLock::new(|| {
    let df = DAT_LOADER
        .write()
        .unwrap()
        ...
        .map(|row| NPCTalkDialogueStepsRow {
            ...
            r#npc_talk_dialogue_text_audio: {
                ...
                let values = df
                    ...
                    .map(|x| x.clone().get_i64_le())
                    .collect::<Vec<i64>>();
                values.into_iter().map(|value| NPCTalkDialogueTextAudioRef::new(value as usize)).collect()
            },
        })
        .collect()
});
...
impl NPCTalkDialogueStepsRef {
    ...
    pub fn iter() -> impl Iterator<Item = Self> {
        TABLE_NPCTalkDialogueSteps.iter().enumerate().map(|(i, _)| Self(i))
    }
    pub fn iter_with_refs() -> impl Iterator<Item = (Self, &'static NPCTalkDialogueStepsRow)> {
        TABLE_NPCTalkDialogueSteps.iter().enumerate().map(|(i, x)| (Self(i), x))
    }
}
