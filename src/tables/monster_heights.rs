...
pub static TABLE_MonsterHeights: LazyLock<Vec<MonsterHeightsRow>> = LazyLock::new(|| {
    let df = DAT_LOADER.write().unwrap().get_table("data/balance/monsterheights.datc64").unwrap().clone();
    df.rows_iter()
...
impl MonsterHeightsRef {
    ...
    pub fn iter() -> impl Iterator<Item = Self> {
        TABLE_MonsterHeights.iter().enumerate().map(|(i, _)| Self(i))
    }
    pub fn iter_with_refs() -> impl Iterator<Item = (Self, &'static MonsterHeightsRow)> {
        TABLE_MonsterHeights.iter().enumerate().map(|(i, x)| (Self(i), x))
    }
}
