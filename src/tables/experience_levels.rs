...
pub static TABLE_ExperienceLevels: LazyLock<Vec<ExperienceLevelsRow>> = LazyLock::new(|| {
    let df = DAT_LOADER.write().unwrap().get_table("data/balance/experiencelevels.datc64").unwrap().clone();
    df.rows_iter()
...
impl ExperienceLevelsRef {
    ...
    pub fn iter() -> impl Iterator<Item = Self> {
        TABLE_ExperienceLevels.iter().enumerate().map(|(i, _)| Self(i))
    }
    pub fn iter_with_refs() -> impl Iterator<Item = (Self, &'static ExperienceLevelsRow)> {
        TABLE_ExperienceLevels.iter().enumerate().map(|(i, x)| (Self(i), x))
    }
}
