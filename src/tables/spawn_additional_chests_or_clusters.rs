...
pub static TABLE_SpawnAdditionalChestsOrClusters: LazyLock<
    Vec<SpawnAdditionalChestsOrClustersRow>,
> = LazyLock::new(|| {
    let df = DAT_LOADER
        .write()
        .unwrap()
...
impl SpawnAdditionalChestsOrClustersRef {
    ...
    pub fn iter() -> impl Iterator<Item = Self> {
        TABLE_SpawnAdditionalChestsOrClusters
            .iter()
            .enumerate()
            .map(|(i, _)| Self(i))
    }
    pub fn iter_with_refs(
    ) -> impl Iterator<Item = (Self, &'static SpawnAdditionalChestsOrClustersRow)> {
        TABLE_SpawnAdditionalChestsOrClusters
            .iter()
            .enumerate()
            .map(|(i, x)| (Self(i), x))
