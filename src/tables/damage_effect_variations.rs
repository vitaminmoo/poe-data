...
pub static TABLE_DamageEffectVariations: LazyLock<Vec<DamageEffectVariationsRow>> = LazyLock::new(|| {
    let df = DAT_LOADER
        .write()
        .unwrap()
...
impl DamageEffectVariationsRef {
    ...
    pub fn iter() -> impl Iterator<Item = Self> {
        TABLE_DamageEffectVariations.iter().enumerate().map(|(i, _)| Self(i))
    }
    pub fn iter_with_refs() -> impl Iterator<Item = (Self, &'static DamageEffectVariationsRow)> {
        TABLE_DamageEffectVariations.iter().enumerate().map(|(i, x)| (Self(i), x))
    }
}
