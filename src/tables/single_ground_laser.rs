...
pub static TABLE_SingleGroundLaser: LazyLock<Vec<SingleGroundLaserRow>> = LazyLock::new(|| {
    let df = DAT_LOADER.write().unwrap().get_table("data/balance/singlegroundlaser.datc64").unwrap().clone();
    df.rows_iter()
...
impl SingleGroundLaserRef {
    ...
    pub fn iter() -> impl Iterator<Item = Self> {
        TABLE_SingleGroundLaser.iter().enumerate().map(|(i, _)| Self(i))
    }
    pub fn iter_with_refs() -> impl Iterator<Item = (Self, &'static SingleGroundLaserRow)> {
        TABLE_SingleGroundLaser.iter().enumerate().map(|(i, x)| (Self(i), x))
    }
}
