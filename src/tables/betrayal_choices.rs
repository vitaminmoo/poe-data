...
pub static TABLE_BetrayalChoices: LazyLock<Vec<BetrayalChoicesRow>> = LazyLock::new(|| {
    let df = DAT_LOADER.write().unwrap().get_table("data/balance/betrayalchoices.datc64").unwrap().clone();
    df.rows_iter()
...
impl BetrayalChoicesRef {
    ...
    pub fn iter() -> impl Iterator<Item = Self> {
        TABLE_BetrayalChoices.iter().enumerate().map(|(i, _)| Self(i))
    }
    pub fn iter_with_refs() -> impl Iterator<Item = (Self, &'static BetrayalChoicesRow)> {
        TABLE_BetrayalChoices.iter().enumerate().map(|(i, x)| (Self(i), x))
    }
}
