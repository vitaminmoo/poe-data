...
pub static TABLE_WieldableClasses: LazyLock<Vec<WieldableClassesRow>> = LazyLock::new(|| {
    let df = DAT_LOADER.write().unwrap().get_table("data/balance/wieldableclasses.datc64").unwrap().clone();
    df.rows_iter()
...
impl WieldableClassesRef {
    ...
    pub fn iter() -> impl Iterator<Item = Self> {
        TABLE_WieldableClasses.iter().enumerate().map(|(i, _)| Self(i))
    }
    pub fn iter_with_refs() -> impl Iterator<Item = (Self, &'static WieldableClassesRow)> {
        TABLE_WieldableClasses.iter().enumerate().map(|(i, x)| (Self(i), x))
    }
}
