...
pub static TABLE_GrantedEffectsPerLevel: LazyLock<Vec<GrantedEffectsPerLevelRow>> = LazyLock::new(|| {
    let df = DAT_LOADER
        .write()
        .unwrap()
...
impl GrantedEffectsPerLevelRef {
    ...
    pub fn iter() -> impl Iterator<Item = Self> {
        TABLE_GrantedEffectsPerLevel.iter().enumerate().map(|(i, _)| Self(i))
    }
    pub fn iter_with_refs() -> impl Iterator<Item = (Self, &'static GrantedEffectsPerLevelRow)> {
        TABLE_GrantedEffectsPerLevel.iter().enumerate().map(|(i, x)| (Self(i), x))
    }
}
