...
pub static TABLE_KillstreakThresholds: LazyLock<Vec<KillstreakThresholdsRow>> = LazyLock::new(|| {
    let df = DAT_LOADER
        .write()
        .unwrap()
...
impl KillstreakThresholdsRef {
    ...
    pub fn iter() -> impl Iterator<Item = Self> {
        TABLE_KillstreakThresholds.iter().enumerate().map(|(i, _)| Self(i))
    }
    pub fn iter_with_refs() -> impl Iterator<Item = (Self, &'static KillstreakThresholdsRow)> {
        TABLE_KillstreakThresholds.iter().enumerate().map(|(i, x)| (Self(i), x))
    }
}
