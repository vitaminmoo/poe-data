...
pub static TABLE_ClientLeagueAction: LazyLock<Vec<ClientLeagueActionRow>> = LazyLock::new(|| {
    let df = DAT_LOADER.write().unwrap().get_table("data/balance/clientleagueaction.datc64").unwrap().clone();
    df.rows_iter()
...
impl ClientLeagueActionRef {
    ...
    pub fn iter() -> impl Iterator<Item = Self> {
        TABLE_ClientLeagueAction.iter().enumerate().map(|(i, _)| Self(i))
    }
    pub fn iter_with_refs() -> impl Iterator<Item = (Self, &'static ClientLeagueActionRow)> {
        TABLE_ClientLeagueAction.iter().enumerate().map(|(i, x)| (Self(i), x))
    }
}
