...
pub static TABLE_BestiaryFamilies: LazyLock<Vec<BestiaryFamiliesRow>> = LazyLock::new(|| {
    let df = DAT_LOADER.write().unwrap().get_table("data/balance/bestiaryfamilies.datc64").unwrap().clone();
    df.rows_iter()
...
impl BestiaryFamiliesRef {
    ...
    pub fn iter() -> impl Iterator<Item = Self> {
        TABLE_BestiaryFamilies.iter().enumerate().map(|(i, _)| Self(i))
    }
    pub fn iter_with_refs() -> impl Iterator<Item = (Self, &'static BestiaryFamiliesRow)> {
        TABLE_BestiaryFamilies.iter().enumerate().map(|(i, x)| (Self(i), x))
    }
}
