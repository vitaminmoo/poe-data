...
pub static TABLE_LegionFactions: LazyLock<Vec<LegionFactionsRow>> = LazyLock::new(|| {
    let df = DAT_LOADER.write().unwrap().get_table("data/balance/legionfactions.datc64").unwrap().clone();
    df.rows_iter()
...
impl LegionFactionsRef {
    ...
    pub fn iter() -> impl Iterator<Item = Self> {
        TABLE_LegionFactions.iter().enumerate().map(|(i, _)| Self(i))
    }
    pub fn iter_with_refs() -> impl Iterator<Item = (Self, &'static LegionFactionsRow)> {
        TABLE_LegionFactions.iter().enumerate().map(|(i, x)| (Self(i), x))
    }
}
