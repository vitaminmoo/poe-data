...
pub static TABLE_MicrotransactionRecycleSalvageValues: LazyLock<
    Vec<MicrotransactionRecycleSalvageValuesRow>,
> = LazyLock::new(|| {
    let df = DAT_LOADER
        .write()
        .unwrap()
...
impl MicrotransactionRecycleSalvageValuesRef {
    ...
    pub fn iter() -> impl Iterator<Item = Self> {
        TABLE_MicrotransactionRecycleSalvageValues
            .iter()
            .enumerate()
            .map(|(i, _)| Self(i))
    }
    pub fn iter_with_refs(
    ) -> impl Iterator<Item = (Self, &'static MicrotransactionRecycleSalvageValuesRow)> {
        TABLE_MicrotransactionRecycleSalvageValues
            .iter()
            .enumerate()
            .map(|(i, x)| (Self(i), x))
