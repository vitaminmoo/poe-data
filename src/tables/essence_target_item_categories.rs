...
pub static TABLE_EssenceTargetItemCategories: LazyLock<Vec<EssenceTargetItemCategoriesRow>> = LazyLock::new(|| {
    let df = DAT_LOADER
        .write()
        .unwrap()
...
impl EssenceTargetItemCategoriesRef {
    ...
    pub fn iter() -> impl Iterator<Item = Self> {
        TABLE_EssenceTargetItemCategories.iter().enumerate().map(|(i, _)| Self(i))
    }
    pub fn iter_with_refs() -> impl Iterator<Item = (Self, &'static EssenceTargetItemCategoriesRow)> {
        TABLE_EssenceTargetItemCategories.iter().enumerate().map(|(i, x)| (Self(i), x))
    }
}
