...
pub static TABLE_TieredMicrotransactionsVisuals: LazyLock<Vec<TieredMicrotransactionsVisualsRow>> = LazyLock::new(|| {
    let df = DAT_LOADER
        .write()
        .unwrap()
...
impl TieredMicrotransactionsVisualsRef {
    ...
    pub fn iter() -> impl Iterator<Item = Self> {
        TABLE_TieredMicrotransactionsVisuals.iter().enumerate().map(|(i, _)| Self(i))
    }
    pub fn iter_with_refs() -> impl Iterator<Item = (Self, &'static TieredMicrotransactionsVisualsRow)> {
        TABLE_TieredMicrotransactionsVisuals.iter().enumerate().map(|(i, x)| (Self(i), x))
    }
}
