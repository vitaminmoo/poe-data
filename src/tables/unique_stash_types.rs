...
pub static TABLE_UniqueStashTypes: LazyLock<Vec<UniqueStashTypesRow>> = LazyLock::new(|| {
    let df = DAT_LOADER.write().unwrap().get_table("data/balance/uniquestashtypes.datc64").unwrap().clone();
    df.rows_iter()
...
impl UniqueStashTypesRef {
    ...
    pub fn iter() -> impl Iterator<Item = Self> {
        TABLE_UniqueStashTypes.iter().enumerate().map(|(i, _)| Self(i))
    }
    pub fn iter_with_refs() -> impl Iterator<Item = (Self, &'static UniqueStashTypesRow)> {
        TABLE_UniqueStashTypes.iter().enumerate().map(|(i, x)| (Self(i), x))
    }
}
