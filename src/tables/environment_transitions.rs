...
pub static TABLE_EnvironmentTransitions: LazyLock<Vec<EnvironmentTransitionsRow>> = LazyLock::new(|| {
    let df = DAT_LOADER
        .write()
        .unwrap()
...
impl EnvironmentTransitionsRef {
    ...
    pub fn iter() -> impl Iterator<Item = Self> {
        TABLE_EnvironmentTransitions.iter().enumerate().map(|(i, _)| Self(i))
    }
    pub fn iter_with_refs() -> impl Iterator<Item = (Self, &'static EnvironmentTransitionsRow)> {
        TABLE_EnvironmentTransitions.iter().enumerate().map(|(i, x)| (Self(i), x))
    }
}
