...
pub static TABLE_MiscProjectileModArtVariations: LazyLock<Vec<MiscProjectileModArtVariationsRow>> = LazyLock::new(|| {
    let df = DAT_LOADER
        .write()
        .unwrap()
        ...
        .map(|row| MiscProjectileModArtVariationsRow {
            ...
            r#unknown32: {
                ...
                let values = df
                    ...
                    .collect::<Vec<i64>>();
                values.into_iter().map(|value| MiscProjectileModRef::new(value as usize)).collect()
            },
        })
        .collect()
});
...
impl MiscProjectileModArtVariationsRef {
    ...
    pub fn iter() -> impl Iterator<Item = Self> {
        TABLE_MiscProjectileModArtVariations.iter().enumerate().map(|(i, _)| Self(i))
    }
    pub fn iter_with_refs() -> impl Iterator<Item = (Self, &'static MiscProjectileModArtVariationsRow)> {
        TABLE_MiscProjectileModArtVariations.iter().enumerate().map(|(i, x)| (Self(i), x))
    }
}
