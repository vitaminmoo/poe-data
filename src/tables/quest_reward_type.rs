...
pub static TABLE_QuestRewardType: LazyLock<Vec<QuestRewardTypeRow>> = LazyLock::new(|| {
    let df = DAT_LOADER.write().unwrap().get_table("data/balance/questrewardtype.datc64").unwrap().clone();
    df.rows_iter()
...
impl QuestRewardTypeRef {
    ...
    pub fn iter() -> impl Iterator<Item = Self> {
        TABLE_QuestRewardType.iter().enumerate().map(|(i, _)| Self(i))
    }
    pub fn iter_with_refs() -> impl Iterator<Item = (Self, &'static QuestRewardTypeRow)> {
        TABLE_QuestRewardType.iter().enumerate().map(|(i, x)| (Self(i), x))
    }
}
