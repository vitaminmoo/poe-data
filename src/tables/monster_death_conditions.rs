...
pub static TABLE_MonsterDeathConditions: LazyLock<Vec<MonsterDeathConditionsRow>> = LazyLock::new(|| {
    let df = DAT_LOADER
        .write()
        .unwrap()
...
impl MonsterDeathConditionsRef {
    ...
    pub fn iter() -> impl Iterator<Item = Self> {
        TABLE_MonsterDeathConditions.iter().enumerate().map(|(i, _)| Self(i))
    }
    pub fn iter_with_refs() -> impl Iterator<Item = (Self, &'static MonsterDeathConditionsRow)> {
        TABLE_MonsterDeathConditions.iter().enumerate().map(|(i, x)| (Self(i), x))
    }
}
