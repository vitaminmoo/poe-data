...
pub static TABLE_GoldRespecPrices: LazyLock<Vec<GoldRespecPricesRow>> = LazyLock::new(|| {
    let df = DAT_LOADER.write().unwrap().get_table("data/balance/goldrespecprices.datc64").unwrap().clone();
    df.rows_iter()
...
impl GoldRespecPricesRef {
    ...
    pub fn iter() -> impl Iterator<Item = Self> {
        TABLE_GoldRespecPrices.iter().enumerate().map(|(i, _)| Self(i))
    }
    pub fn iter_with_refs() -> impl Iterator<Item = (Self, &'static GoldRespecPricesRow)> {
        TABLE_GoldRespecPrices.iter().enumerate().map(|(i, x)| (Self(i), x))
    }
}
