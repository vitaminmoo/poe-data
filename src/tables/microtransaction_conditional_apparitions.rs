...
pub static TABLE_MicrotransactionConditionalApparitions: LazyLock<Vec<MicrotransactionConditionalApparitionsRow>> = LazyLock::new(|| {
    let df = DAT_LOADER
        .write()
        .unwrap()
...
impl MicrotransactionConditionalApparitionsRef {
    ...
    pub fn iter() -> impl Iterator<Item = Self> {
        TABLE_MicrotransactionConditionalApparitions.iter().enumerate().map(|(i, _)| Self(i))
    }
    pub fn iter_with_refs() -> impl Iterator<Item = (Self, &'static MicrotransactionConditionalApparitionsRow)> {
        TABLE_MicrotransactionConditionalApparitions.iter().enumerate().map(|(i, x)| (Self(i), x))
    }
}
