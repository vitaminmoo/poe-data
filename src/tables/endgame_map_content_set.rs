...
pub static TABLE_EndgameMapContentSet: LazyLock<Vec<EndgameMapContentSetRow>> = LazyLock::new(|| {
    let df = DAT_LOADER
        .write()
        .unwrap()
        ...
        .map(|row| EndgameMapContentSetRow {
            ...
            r#content: {
                ...
                let values = df
                    ...
                    .collect::<Vec<i64>>();
                values.into_iter().map(|value| EndgameMapContentRef::new(value as usize)).collect()
            },
        })
        .collect()
});
...
impl EndgameMapContentSetRef {
    ...
    pub fn iter() -> impl Iterator<Item = Self> {
        TABLE_EndgameMapContentSet.iter().enumerate().map(|(i, _)| Self(i))
    }
    pub fn iter_with_refs() -> impl Iterator<Item = (Self, &'static EndgameMapContentSetRow)> {
        TABLE_EndgameMapContentSet.iter().enumerate().map(|(i, x)| (Self(i), x))
    }
}
