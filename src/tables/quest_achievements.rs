...
pub static TABLE_QuestAchievements: LazyLock<Vec<QuestAchievementsRow>> = LazyLock::new(|| {
    let df = DAT_LOADER.write().unwrap().get_table("data/balance/questachievements.datc64").unwrap().clone();
    df.rows_iter()
...
impl QuestAchievementsRef {
    ...
    pub fn iter() -> impl Iterator<Item = Self> {
        TABLE_QuestAchievements.iter().enumerate().map(|(i, _)| Self(i))
    }
    pub fn iter_with_refs() -> impl Iterator<Item = (Self, &'static QuestAchievementsRow)> {
        TABLE_QuestAchievements.iter().enumerate().map(|(i, x)| (Self(i), x))
    }
}
