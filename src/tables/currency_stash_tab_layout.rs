...
pub static TABLE_CurrencyStashTabLayout: LazyLock<Vec<CurrencyStashTabLayoutRow>> = LazyLock::new(|| {
    let df = DAT_LOADER
        .write()
        .unwrap()
...
impl CurrencyStashTabLayoutRef {
    ...
    pub fn iter() -> impl Iterator<Item = Self> {
        TABLE_CurrencyStashTabLayout.iter().enumerate().map(|(i, _)| Self(i))
    }
    pub fn iter_with_refs() -> impl Iterator<Item = (Self, &'static CurrencyStashTabLayoutRow)> {
        TABLE_CurrencyStashTabLayout.iter().enumerate().map(|(i, x)| (Self(i), x))
    }
}
