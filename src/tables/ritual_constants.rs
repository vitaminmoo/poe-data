...
pub static TABLE_RitualConstants: LazyLock<Vec<RitualConstantsRow>> = LazyLock::new(|| {
    let df = DAT_LOADER.write().unwrap().get_table("data/balance/ritualconstants.datc64").unwrap().clone();
    df.rows_iter()
...
impl RitualConstantsRef {
    ...
    pub fn iter() -> impl Iterator<Item = Self> {
        TABLE_RitualConstants.iter().enumerate().map(|(i, _)| Self(i))
    }
    pub fn iter_with_refs() -> impl Iterator<Item = (Self, &'static RitualConstantsRow)> {
        TABLE_RitualConstants.iter().enumerate().map(|(i, x)| (Self(i), x))
    }
}
