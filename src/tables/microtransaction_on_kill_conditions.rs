...
pub static TABLE_MicrotransactionOnKillConditions: LazyLock<
    Vec<MicrotransactionOnKillConditionsRow>,
> = LazyLock::new(|| {
    let df = DAT_LOADER
        .write()
        .unwrap()
...
impl MicrotransactionOnKillConditionsRef {
    ...
    pub fn iter() -> impl Iterator<Item = Self> {
        TABLE_MicrotransactionOnKillConditions
            .iter()
            .enumerate()
            .map(|(i, _)| Self(i))
    }
    pub fn iter_with_refs(
    ) -> impl Iterator<Item = (Self, &'static MicrotransactionOnKillConditionsRow)> {
        TABLE_MicrotransactionOnKillConditions
            .iter()
            .enumerate()
            .map(|(i, x)| (Self(i), x))
