...
pub static TABLE_MicrotransactionSlotAdditionalDefaultOptions: LazyLock<Vec<MicrotransactionSlotAdditionalDefaultOptionsRow>> = LazyLock::new(|| {
    let df = DAT_LOADER
        .write()
        .unwrap()
...
impl MicrotransactionSlotAdditionalDefaultOptionsRef {
    ...
    pub fn iter() -> impl Iterator<Item = Self> {
        TABLE_MicrotransactionSlotAdditionalDefaultOptions.iter().enumerate().map(|(i, _)| Self(i))
    }
    pub fn iter_with_refs() -> impl Iterator<Item = (Self, &'static MicrotransactionSlotAdditionalDefaultOptionsRow)> {
        TABLE_MicrotransactionSlotAdditionalDefaultOptions.iter().enumerate().map(|(i, x)| (Self(i), x))
    }
}
