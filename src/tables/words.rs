...
pub static TABLE_Words: LazyLock<Vec<WordsRow>> = LazyLock::new(|| {
    let df = DAT_LOADER.write().unwrap().get_table("data/balance/words.datc64").unwrap().clone();
    df.rows_iter()
        .map(|row| WordsRow {
            r#wordlist: {
                ...
                Wordlists::from_repr(value as usize)
            },
            r#text: {
...
#[derive(Debug)]
pub struct WordsRow {
    pub r#wordlist: Option<Wordlists>,
    pub r#text: String,
...
pub struct WordsRef(pub usize);

impl Deref for WordsRef {
    type Target = WordsRow;
...
impl WordsRef {
    ...
    pub fn as_static_ref(&self) -> &'static WordsRow {
        &TABLE_Words[self.0]
    }
    pub fn get(&self) -> &'static WordsRow {
        &TABLE_Words[self.0]
    }
    pub fn iter() -> impl Iterator<Item = Self> {
        TABLE_Words.iter().enumerate().map(|(i, _)| Self(i))
    }
    pub fn iter_with_refs() -> impl Iterator<Item = (Self, &'static WordsRow)> {
