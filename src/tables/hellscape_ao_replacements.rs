...
pub struct HellscapeAOReplacementsRow {
    pub r#original: String,
    pub r#hash32: i32,
    pub r#replacement: String,
}

#[derive(Debug, Clone, Copy, Hash, PartialEq, Eq, Ord, PartialOrd)]
pub struct HellscapeAOReplacementsRef(pub usize);

impl Deref for HellscapeAOReplacementsRef {
    type Target = HellscapeAOReplacementsRow;
    fn deref(&self) -> &'static Self::Target {
        &TABLE_HellscapeAOReplacements[self.0]
    }
}

impl HellscapeAOReplacementsRef {
    pub fn new(index: usize) -> Self {
        Self(index)
    }
    pub fn as_static_ref(&self) -> &'static HellscapeAOReplacementsRow {
        &TABLE_HellscapeAOReplacements[self.0]
    }
    pub fn get(&self) -> &'static HellscapeAOReplacementsRow {
        &TABLE_HellscapeAOReplacements[self.0]
    }
...
mod test {
    ...
    #[test]
    fn get_all_rows() {
        for row in TABLE_HellscapeAOReplacements.iter() {
            black_box(row);
        }
    }
}
