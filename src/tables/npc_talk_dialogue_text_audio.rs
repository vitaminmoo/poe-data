...
pub static TABLE_NPCTalkDialogueTextAudio: LazyLock<Vec<NPCTalkDialogueTextAudioRow>> = LazyLock::new(|| {
    let df = DAT_LOADER
        .write()
        .unwrap()
...
impl NPCTalkDialogueTextAudioRef {
    ...
    pub fn iter() -> impl Iterator<Item = Self> {
        TABLE_NPCTalkDialogueTextAudio.iter().enumerate().map(|(i, _)| Self(i))
    }
    pub fn iter_with_refs() -> impl Iterator<Item = (Self, &'static NPCTalkDialogueTextAudioRow)> {
        TABLE_NPCTalkDialogueTextAudio.iter().enumerate().map(|(i, x)| (Self(i), x))
    }
}
