...
pub static TABLE_MonsterProjectileSpell: LazyLock<Vec<MonsterProjectileSpellRow>> = LazyLock::new(|| {
    let df = DAT_LOADER
        .write()
        .unwrap()
...
impl MonsterProjectileSpellRef {
    ...
    pub fn iter() -> impl Iterator<Item = Self> {
        TABLE_MonsterProjectileSpell.iter().enumerate().map(|(i, _)| Self(i))
    }
    pub fn iter_with_refs() -> impl Iterator<Item = (Self, &'static MonsterProjectileSpellRow)> {
        TABLE_MonsterProjectileSpell.iter().enumerate().map(|(i, x)| (Self(i), x))
    }
}
