...
pub static TABLE_TryTheNewLeagueVersions: LazyLock<Vec<TryTheNewLeagueVersionsRow>> = LazyLock::new(|| {
    let df = DAT_LOADER
        .write()
        .unwrap()
...
impl TryTheNewLeagueVersionsRef {
    ...
    pub fn iter() -> impl Iterator<Item = Self> {
        TABLE_TryTheNewLeagueVersions.iter().enumerate().map(|(i, _)| Self(i))
    }
    pub fn iter_with_refs() -> impl Iterator<Item = (Self, &'static TryTheNewLeagueVersionsRow)> {
        TABLE_TryTheNewLeagueVersions.iter().enumerate().map(|(i, x)| (Self(i), x))
    }
}
