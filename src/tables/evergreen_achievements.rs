...
pub static TABLE_EvergreenAchievements: LazyLock<Vec<EvergreenAchievementsRow>> = LazyLock::new(|| {
    let df = DAT_LOADER
        .write()
        .unwrap()
...
impl EvergreenAchievementsRef {
    ...
    pub fn iter() -> impl Iterator<Item = Self> {
        TABLE_EvergreenAchievements.iter().enumerate().map(|(i, _)| Self(i))
    }
    pub fn iter_with_refs() -> impl Iterator<Item = (Self, &'static EvergreenAchievementsRow)> {
        TABLE_EvergreenAchievements.iter().enumerate().map(|(i, x)| (Self(i), x))
    }
}
