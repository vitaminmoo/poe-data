...
pub static TABLE_StashTabAffinityByBaseItemType: LazyLock<Vec<StashTabAffinityByBaseItemTypeRow>> =
    LazyLock::new(|| {
        let df = DAT_LOADER
            .write()
            .unwrap()
        ...
        df.rows_iter()
            .map(|row| StashTabAffinityByBaseItemTypeRow {
                ...
                r#stash_tab_affinity_id: {
                    ...
                    values
                        .into_iter()
                        .map(|value| StashTabAffinityIdRef::new(value as usize))
                        .collect()
                },
            })
            .collect()
...
impl StashTabAffinityByBaseItemTypeRef {
    ...
    pub fn iter() -> impl Iterator<Item = Self> {
        TABLE_StashTabAffinityByBaseItemType
            .iter()
            .enumerate()
            .map(|(i, _)| Self(i))
    }
    pub fn iter_with_refs(
    ) -> impl Iterator<Item = (Self, &'static StashTabAffinityByBaseItemTypeRow)> {
        TABLE_StashTabAffinityByBaseItemType
            .iter()
            .enumerate()
            .map(|(i, x)| (Self(i), x))
