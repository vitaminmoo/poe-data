...
pub static TABLE_AfflictionSplitDemons: LazyLock<Vec<AfflictionSplitDemonsRow>> = LazyLock::new(|| {
    let df = DAT_LOADER
        .write()
        .unwrap()
...
impl AfflictionSplitDemonsRef {
    ...
    pub fn iter() -> impl Iterator<Item = Self> {
        TABLE_AfflictionSplitDemons.iter().enumerate().map(|(i, _)| Self(i))
    }
    pub fn iter_with_refs() -> impl Iterator<Item = (Self, &'static AfflictionSplitDemonsRow)> {
        TABLE_AfflictionSplitDemons.iter().enumerate().map(|(i, x)| (Self(i), x))
    }
}
