...
pub static TABLE_StatisticTrackingMicrotransactions: LazyLock<Vec<StatisticTrackingMicrotransactionsRow>> = LazyLock::new(|| {
    let df = DAT_LOADER
        .write()
        .unwrap()
...
impl StatisticTrackingMicrotransactionsRef {
    ...
    pub fn iter() -> impl Iterator<Item = Self> {
        TABLE_StatisticTrackingMicrotransactions.iter().enumerate().map(|(i, _)| Self(i))
    }
    pub fn iter_with_refs() -> impl Iterator<Item = (Self, &'static StatisticTrackingMicrotransactionsRow)> {
        TABLE_StatisticTrackingMicrotransactions.iter().enumerate().map(|(i, x)| (Self(i), x))
    }
}
