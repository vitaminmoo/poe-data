...
pub static TABLE_BlightChestTypes: LazyLock<Vec<BlightChestTypesRow>> = LazyLock::new(|| {
    let df = DAT_LOADER.write().unwrap().get_table("data/balance/blightchesttypes.datc64").unwrap().clone();
    df.rows_iter()
...
impl BlightChestTypesRef {
    ...
    pub fn iter() -> impl Iterator<Item = Self> {
        TABLE_BlightChestTypes.iter().enumerate().map(|(i, _)| Self(i))
    }
    pub fn iter_with_refs() -> impl Iterator<Item = (Self, &'static BlightChestTypesRow)> {
        TABLE_BlightChestTypes.iter().enumerate().map(|(i, x)| (Self(i), x))
    }
}
