...
pub static TABLE_BuffVisualsArtVariations: LazyLock<Vec<BuffVisualsArtVariationsRow>> = LazyLock::new(|| {
    let df = DAT_LOADER
        .write()
        .unwrap()
...
impl BuffVisualsArtVariationsRef {
    ...
    pub fn iter() -> impl Iterator<Item = Self> {
        TABLE_BuffVisualsArtVariations.iter().enumerate().map(|(i, _)| Self(i))
    }
    pub fn iter_with_refs() -> impl Iterator<Item = (Self, &'static BuffVisualsArtVariationsRow)> {
        TABLE_BuffVisualsArtVariations.iter().enumerate().map(|(i, x)| (Self(i), x))
    }
}
