...
pub static TABLE_MicrotransactionCursorVariations: LazyLock<Vec<MicrotransactionCursorVariationsRow>> = LazyLock::new(|| {
    let df = DAT_LOADER
        .write()
        .unwrap()
...
impl MicrotransactionCursorVariationsRef {
    ...
    pub fn iter() -> impl Iterator<Item = Self> {
        TABLE_MicrotransactionCursorVariations.iter().enumerate().map(|(i, _)| Self(i))
    }
    pub fn iter_with_refs() -> impl Iterator<Item = (Self, &'static MicrotransactionCursorVariationsRow)> {
        TABLE_MicrotransactionCursorVariations.iter().enumerate().map(|(i, x)| (Self(i), x))
    }
}
