...
pub static TABLE_SanctumPersistentEffects: LazyLock<Vec<SanctumPersistentEffectsRow>> = LazyLock::new(|| {
    let df = DAT_LOADER
        .write()
        .unwrap()
...
impl SanctumPersistentEffectsRef {
    ...
    pub fn iter() -> impl Iterator<Item = Self> {
        TABLE_SanctumPersistentEffects.iter().enumerate().map(|(i, _)| Self(i))
    }
    pub fn iter_with_refs() -> impl Iterator<Item = (Self, &'static SanctumPersistentEffectsRow)> {
        TABLE_SanctumPersistentEffects.iter().enumerate().map(|(i, x)| (Self(i), x))
    }
}
