#![allow(clippy::all)]
use bytes::Buf;

use crate::dat_parser::DAT_LOADER;

#[allow(unused)]
use super::*;
use std::{ops::Deref, sync::LazyLock};

#[allow(non_upper_case_globals)]
pub static TABLE_GemEffects: LazyLock<Vec<GemEffectsRow>> = LazyLock::new(|| {
    let df = DAT_LOADER
        .write()
        .unwrap()
        .get_table("data/balance/gemeffects.datc64")
        .unwrap()
        .clone();
    df.rows_iter()
        .map(|row| GemEffectsRow {
            r#id: {
                // array_mutator column.array == false && column.type == 'string'
                let mut cell_bytes = row.get(0..0 + 8).unwrap();
                let offset = cell_bytes.get_i32_le() as usize;
                let value = df.string_from_offset(offset).unwrap();
                value
            },
            r#name: {
                // array_mutator column.array == false && column.type == 'string'
                let mut cell_bytes = row.get(8..8 + 8).unwrap();
                let offset = cell_bytes.get_i32_le() as usize;
                let value = df.string_from_offset(offset).unwrap();
                value
            },
            r#granted_effect: {
                // array_mutator column.array == false && column.type != 'string|bool'
                let mut cell_bytes = row.get(16..16 + 16).unwrap();
                let value = cell_bytes.get_i64_le();
                GrantedEffectsRef::new(value as usize)
            },
            r#support_text: {
                // array_mutator column.array == false && column.type == 'string'
                let mut cell_bytes = row.get(32..32 + 8).unwrap();
                let offset = cell_bytes.get_i32_le() as usize;
                let value = df.string_from_offset(offset).unwrap();
                value
            },
            r#support_name: {
                // array_mutator column.array == false && column.type == 'string'
                let mut cell_bytes = row.get(40..40 + 8).unwrap();
                let offset = cell_bytes.get_i32_le() as usize;
                let value = df.string_from_offset(offset).unwrap();
                value
            },
            r#gem_tags: {
                // array_mutator column.array == true
                let mut cell_bytes = row.get(48..48 + 16).unwrap();
                let count = cell_bytes.get_u64_le() as usize;
                let offset = cell_bytes.get_u64_le() as usize;
                // array_mutator column.array == true && column.type else
                let values = df
                    .array_from_offset(offset, count, 16)
                    .unwrap()
                    .iter()
                    .map(|x| x.clone().get_i64_le())
                    .collect::<Vec<i64>>();
                values
                    .into_iter()
                    .map(|value| GemTagsRef::new(value as usize))
                    .collect()
            },
            r#consumed_mod: {
                // array_mutator column.array == false && column.type != 'string|bool'
                let mut cell_bytes = row.get(64..64 + 16).unwrap();
                let value = cell_bytes.get_i64_le();
                ModsRef::new(value as usize)
            },
            r#item_color: {
                // array_mutator column.array == false && column.type != 'string|bool'
                let mut cell_bytes = row.get(80..80 + 4).unwrap();
                let value = cell_bytes.get_i32_le();
                value
            },
            r#unknown84: {
                // array_mutator column.array == true
                let mut cell_bytes = row.get(84..84 + 16).unwrap();
                let count = cell_bytes.get_u64_le() as usize;
                let offset = cell_bytes.get_u64_le() as usize;
                // array_mutator column.array == true && column.type else
                let values = df
                    .array_from_offset(offset, count, 16)
                    .unwrap()
                    .iter()
                    .map(|x| x.clone().get_i64_le())
                    .collect::<Vec<i64>>();
                values
            },
            r#additional_granted_effects: {
                // array_mutator column.array == true
                let mut cell_bytes = row.get(100..100 + 16).unwrap();
                let count = cell_bytes.get_u64_le() as usize;
                let offset = cell_bytes.get_u64_le() as usize;
                // array_mutator column.array == true && column.type else
                let values = df
                    .array_from_offset(offset, count, 16)
                    .unwrap()
                    .iter()
                    .map(|x| x.clone().get_i64_le())
                    .collect::<Vec<i64>>();
                values
                    .into_iter()
                    .map(|value| GrantedEffectsRef::new(value as usize))
                    .collect()
            },
            r#unknown116: {
                // array_mutator column.array == false && column.type == 'bool'
                let cell_bytes = row.get(116).unwrap();
                let value = cell_bytes.to_le() != 0;
                value
            },
            r#unknown117: {
                // array_mutator column.array == false && column.type == 'bool'
                let cell_bytes = row.get(117).unwrap();
                let value = cell_bytes.to_le() != 0;
                value
            },
            r#unknown118: {
                // array_mutator column.array == true
                let mut cell_bytes = row.get(118..118 + 16).unwrap();
                let count = cell_bytes.get_u64_le() as usize;
                let offset = cell_bytes.get_u64_le() as usize;
                // array_mutator column.array == true && column.type else
                let values = df
                    .array_from_offset(offset, count, 4)
                    .unwrap()
                    .iter()
                    .map(|x| x.clone().get_i32_le())
                    .collect::<Vec<i32>>();
                values
            },
            r#unknown134: {
                // array_mutator column.array == false && column.type != 'string|bool'
                let mut cell_bytes = row.get(134..134 + 4).unwrap();
                let value = cell_bytes.get_i32_le();
                value
            },
            r#unknown138: {
                // array_mutator column.array == false && column.type != 'string|bool'
                let mut cell_bytes = row.get(138..138 + 4).unwrap();
                let value = cell_bytes.get_i32_le();
                value
            },
        })
        .collect()
});

#[derive(Debug)]
pub struct GemEffectsRow {
    pub r#id: String,
    pub r#name: String,
    pub r#granted_effect: GrantedEffectsRef,
    pub r#support_text: String,
    pub r#support_name: String,
    pub r#gem_tags: Vec<GemTagsRef>,
    pub r#consumed_mod: ModsRef,
    pub r#item_color: i32,
    pub r#unknown84: Vec<i64>,
    pub r#additional_granted_effects: Vec<GrantedEffectsRef>,
    pub r#unknown116: bool,
    pub r#unknown117: bool,
    pub r#unknown118: Vec<i32>,
    pub r#unknown134: i32,
    pub r#unknown138: i32,
}

#[derive(Debug, Clone, Copy, Hash, PartialEq, Eq, Ord, PartialOrd)]
pub struct GemEffectsRef(pub usize);

impl Deref for GemEffectsRef {
    type Target = GemEffectsRow;
    fn deref(&self) -> &'static Self::Target {
        &TABLE_GemEffects[self.0]
    }
}

impl GemEffectsRef {
    pub fn new(index: usize) -> Self {
        Self(index)
    }
    pub fn as_static_ref(&self) -> &'static GemEffectsRow {
        &TABLE_GemEffects[self.0]
    }
    pub fn get(&self) -> &'static GemEffectsRow {
        &TABLE_GemEffects[self.0]
    }
    pub fn iter() -> impl Iterator<Item = Self> {
        TABLE_GemEffects.iter().enumerate().map(|(i, _)| Self(i))
    }
    pub fn iter_with_refs() -> impl Iterator<Item = (Self, &'static GemEffectsRow)> {
        TABLE_GemEffects
            .iter()
            .enumerate()
            .map(|(i, x)| (Self(i), x))
    }
}

#[cfg(test)]
mod test {
    use super::*;
    use std::hint::black_box;
    #[test]
    fn get_all_rows() {
        for row in TABLE_GemEffects.iter() {
            black_box(row);
        }
    }
}
