...
pub static TABLE_ItemisedVisualEffect: LazyLock<Vec<ItemisedVisualEffectRow>> = LazyLock::new(|| {
    let df = DAT_LOADER
        .write()
        .unwrap()
...
impl ItemisedVisualEffectRef {
    ...
    pub fn iter() -> impl Iterator<Item = Self> {
        TABLE_ItemisedVisualEffect.iter().enumerate().map(|(i, _)| Self(i))
    }
    pub fn iter_with_refs() -> impl Iterator<Item = (Self, &'static ItemisedVisualEffectRow)> {
        TABLE_ItemisedVisualEffect.iter().enumerate().map(|(i, x)| (Self(i), x))
    }
}
