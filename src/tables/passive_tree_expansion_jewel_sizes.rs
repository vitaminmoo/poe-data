...
pub static TABLE_PassiveTreeExpansionJewelSizes: LazyLock<Vec<PassiveTreeExpansionJewelSizesRow>> = LazyLock::new(|| {
    let df = DAT_LOADER
        .write()
        .unwrap()
...
impl PassiveTreeExpansionJewelSizesRef {
    ...
    pub fn iter() -> impl Iterator<Item = Self> {
        TABLE_PassiveTreeExpansionJewelSizes.iter().enumerate().map(|(i, _)| Self(i))
    }
    pub fn iter_with_refs() -> impl Iterator<Item = (Self, &'static PassiveTreeExpansionJewelSizesRow)> {
        TABLE_PassiveTreeExpansionJewelSizes.iter().enumerate().map(|(i, x)| (Self(i), x))
    }
}
