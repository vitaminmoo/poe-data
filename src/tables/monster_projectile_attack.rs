...
pub static TABLE_MonsterProjectileAttack: LazyLock<Vec<MonsterProjectileAttackRow>> = LazyLock::new(|| {
    let df = DAT_LOADER
        .write()
        .unwrap()
...
impl MonsterProjectileAttackRef {
    ...
    pub fn iter() -> impl Iterator<Item = Self> {
        TABLE_MonsterProjectileAttack.iter().enumerate().map(|(i, _)| Self(i))
    }
    pub fn iter_with_refs() -> impl Iterator<Item = (Self, &'static MonsterProjectileAttackRow)> {
        TABLE_MonsterProjectileAttack.iter().enumerate().map(|(i, x)| (Self(i), x))
    }
}
