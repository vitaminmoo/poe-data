...
pub static TABLE_SupportGemSocketedVisualIdentity: LazyLock<Vec<SupportGemSocketedVisualIdentityRow>> = LazyLock::new(|| {
    let df = DAT_LOADER
        .write()
        .unwrap()
...
impl SupportGemSocketedVisualIdentityRef {
    ...
    pub fn iter() -> impl Iterator<Item = Self> {
        TABLE_SupportGemSocketedVisualIdentity.iter().enumerate().map(|(i, _)| Self(i))
    }
    pub fn iter_with_refs() -> impl Iterator<Item = (Self, &'static SupportGemSocketedVisualIdentityRow)> {
        TABLE_SupportGemSocketedVisualIdentity.iter().enumerate().map(|(i, x)| (Self(i), x))
    }
}
