...
pub static TABLE_HideoutCraftingBenchInterfaceVisuals: LazyLock<
    Vec<HideoutCraftingBenchInterfaceVisualsRow>,
> = LazyLock::new(|| {
    let df = DAT_LOADER
        .write()
        .unwrap()
...
impl HideoutCraftingBenchInterfaceVisualsRef {
    ...
    pub fn iter() -> impl Iterator<Item = Self> {
        TABLE_HideoutCraftingBenchInterfaceVisuals
            .iter()
            .enumerate()
            .map(|(i, _)| Self(i))
    }
    pub fn iter_with_refs(
    ) -> impl Iterator<Item = (Self, &'static HideoutCraftingBenchInterfaceVisualsRow)> {
        TABLE_HideoutCraftingBenchInterfaceVisuals
            .iter()
            .enumerate()
            .map(|(i, x)| (Self(i), x))
