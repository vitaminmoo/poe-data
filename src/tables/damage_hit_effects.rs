...
pub static TABLE_DamageHitEffects: LazyLock<Vec<DamageHitEffectsRow>> = LazyLock::new(|| {
    let df = DAT_LOADER.write().unwrap().get_table("data/balance/damagehiteffects.datc64").unwrap().clone();
    df.rows_iter()
...
impl DamageHitEffectsRef {
    ...
    pub fn iter() -> impl Iterator<Item = Self> {
        TABLE_DamageHitEffects.iter().enumerate().map(|(i, _)| Self(i))
    }
    pub fn iter_with_refs() -> impl Iterator<Item = (Self, &'static DamageHitEffectsRow)> {
        TABLE_DamageHitEffects.iter().enumerate().map(|(i, x)| (Self(i), x))
    }
}
