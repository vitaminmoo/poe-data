...
pub static TABLE_CharacterStartQuestState: LazyLock<Vec<CharacterStartQuestStateRow>> = LazyLock::new(|| {
    let df = DAT_LOADER
        .write()
        .unwrap()
        ...
        .map(|row| CharacterStartQuestStateRow {
            ...
            r#quest_keys: {
                ...
                let values = df
                    ...
                    .collect::<Vec<i64>>();
                values.into_iter().map(|value| QuestRef::new(value as usize)).collect()
            },
            r#quest_states: {
                // array_mutator column.array == true
...
impl CharacterStartQuestStateRef {
    ...
    pub fn iter() -> impl Iterator<Item = Self> {
        TABLE_CharacterStartQuestState.iter().enumerate().map(|(i, _)| Self(i))
    }
    pub fn iter_with_refs() -> impl Iterator<Item = (Self, &'static CharacterStartQuestStateRow)> {
        TABLE_CharacterStartQuestState.iter().enumerate().map(|(i, x)| (Self(i), x))
    }
}
