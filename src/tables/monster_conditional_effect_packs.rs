...
pub static TABLE_MonsterConditionalEffectPacks: LazyLock<Vec<MonsterConditionalEffectPacksRow>> = LazyLock::new(|| {
    let df = DAT_LOADER
        .write()
        .unwrap()
        ...
        .map(|row| MonsterConditionalEffectPacksRow {
            ...
            r#misc_effect_pack1: {
                ...
                let values = df
                    ...
                    .collect::<Vec<i64>>();
                values.into_iter().map(|value| MiscEffectPacksRef::new(value as usize)).collect()
            },
            r#misc_effect_pack2: {
                // array_mutator column.array == true
...
impl MonsterConditionalEffectPacksRef {
    ...
    pub fn iter() -> impl Iterator<Item = Self> {
        TABLE_MonsterConditionalEffectPacks.iter().enumerate().map(|(i, _)| Self(i))
    }
    pub fn iter_with_refs() -> impl Iterator<Item = (Self, &'static MonsterConditionalEffectPacksRow)> {
        TABLE_MonsterConditionalEffectPacks.iter().enumerate().map(|(i, x)| (Self(i), x))
    }
}
