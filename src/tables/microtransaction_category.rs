...
pub static TABLE_MicrotransactionCategory: LazyLock<Vec<MicrotransactionCategoryRow>> = LazyLock::new(|| {
    let df = DAT_LOADER
        .write()
        .unwrap()
...
impl MicrotransactionCategoryRef {
    ...
    pub fn iter() -> impl Iterator<Item = Self> {
        TABLE_MicrotransactionCategory.iter().enumerate().map(|(i, _)| Self(i))
    }
    pub fn iter_with_refs() -> impl Iterator<Item = (Self, &'static MicrotransactionCategoryRow)> {
        TABLE_MicrotransactionCategory.iter().enumerate().map(|(i, x)| (Self(i), x))
    }
}
