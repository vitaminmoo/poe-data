...
pub static TABLE_RitualRuneTypes: LazyLock<Vec<RitualRuneTypesRow>> = LazyLock::new(|| {
    let df = DAT_LOADER.write().unwrap().get_table("data/balance/ritualrunetypes.datc64").unwrap().clone();
    df.rows_iter()
        .map(|row| RitualRuneTypesRow {
            ...
            r#spawn_patterns: {
                ...
                let values = df
                    ...
                    .collect::<Vec<i64>>();
                values.into_iter().map(|value| RitualSpawnPatternsRef::new(value as usize)).collect()
            },
            r#mods_key: {
                // array_mutator column.array == true
...
impl RitualRuneTypesRef {
    ...
    pub fn iter() -> impl Iterator<Item = Self> {
        TABLE_RitualRuneTypes.iter().enumerate().map(|(i, _)| Self(i))
    }
    pub fn iter_with_refs() -> impl Iterator<Item = (Self, &'static RitualRuneTypesRow)> {
        TABLE_RitualRuneTypes.iter().enumerate().map(|(i, x)| (Self(i), x))
    }
}
