...
pub static TABLE_MonsterDeathAchievements: LazyLock<Vec<MonsterDeathAchievementsRow>> =
    LazyLock::new(|| {
        let df = DAT_LOADER
            .write()
            .unwrap()
        ...
        df.rows_iter()
            .map(|row| MonsterDeathAchievementsRow {
                ...
                r#player_conditions_keys: {
                    ...
                    values
                        .into_iter()
                        .map(|value| PlayerConditionsRef::new(value as usize))
                        .collect()
                },
                r#monster_death_conditions_keys: {
                    ...
                    values
                        .into_iter()
                        .map(|value| MonsterDeathConditionsRef::new(value as usize))
                        .collect()
                },
                r#unknown73: {
                    // array_mutator column.array == true
...
impl MonsterDeathAchievementsRef {
    ...
    pub fn iter() -> impl Iterator<Item = Self> {
        TABLE_MonsterDeathAchievements
            .iter()
            .enumerate()
            .map(|(i, _)| Self(i))
    }
