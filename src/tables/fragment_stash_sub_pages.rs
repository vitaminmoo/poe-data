...
pub static TABLE_FragmentStashSubPages: LazyLock<Vec<FragmentStashSubPagesRow>> = LazyLock::new(|| {
    let df = DAT_LOADER
        .write()
        .unwrap()
...
impl FragmentStashSubPagesRef {
    ...
    pub fn iter() -> impl Iterator<Item = Self> {
        TABLE_FragmentStashSubPages.iter().enumerate().map(|(i, _)| Self(i))
    }
    pub fn iter_with_refs() -> impl Iterator<Item = (Self, &'static FragmentStashSubPagesRow)> {
        TABLE_FragmentStashSubPages.iter().enumerate().map(|(i, x)| (Self(i), x))
    }
}
