...
pub static TABLE_AtlasPrimordialBossOptions: LazyLock<Vec<AtlasPrimordialBossOptionsRow>> = LazyLock::new(|| {
    let df = DAT_LOADER
        .write()
        .unwrap()
...
impl AtlasPrimordialBossOptionsRef {
    ...
    pub fn iter() -> impl Iterator<Item = Self> {
        TABLE_AtlasPrimordialBossOptions.iter().enumerate().map(|(i, _)| Self(i))
    }
    pub fn iter_with_refs() -> impl Iterator<Item = (Self, &'static AtlasPrimordialBossOptionsRow)> {
        TABLE_AtlasPrimordialBossOptions.iter().enumerate().map(|(i, x)| (Self(i), x))
    }
}
