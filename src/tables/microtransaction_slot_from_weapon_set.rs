...
pub static TABLE_MicrotransactionSlotFromWeaponSet: LazyLock<
    Vec<MicrotransactionSlotFromWeaponSetRow>,
> = LazyLock::new(|| {
    let df = DAT_LOADER
        .write()
        .unwrap()
...
impl MicrotransactionSlotFromWeaponSetRef {
    ...
    pub fn iter() -> impl Iterator<Item = Self> {
        TABLE_MicrotransactionSlotFromWeaponSet
            .iter()
            .enumerate()
            .map(|(i, _)| Self(i))
    }
    pub fn iter_with_refs(
    ) -> impl Iterator<Item = (Self, &'static MicrotransactionSlotFromWeaponSetRow)> {
        TABLE_MicrotransactionSlotFromWeaponSet
            .iter()
            .enumerate()
            .map(|(i, x)| (Self(i), x))
