...
pub static TABLE_GrantedEffectStatSets: LazyLock<Vec<GrantedEffectStatSetsRow>> = LazyLock::new(|| {
    let df = DAT_LOADER
        .write()
        .unwrap()
...
impl GrantedEffectStatSetsRef {
    ...
    pub fn iter() -> impl Iterator<Item = Self> {
        TABLE_GrantedEffectStatSets.iter().enumerate().map(|(i, _)| Self(i))
    }
    pub fn iter_with_refs() -> impl Iterator<Item = (Self, &'static GrantedEffectStatSetsRow)> {
        TABLE_GrantedEffectStatSets.iter().enumerate().map(|(i, x)| (Self(i), x))
    }
}
