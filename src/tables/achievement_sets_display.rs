...
pub static TABLE_AchievementSetsDisplay: LazyLock<Vec<AchievementSetsDisplayRow>> = LazyLock::new(|| {
    let df = DAT_LOADER
        .write()
        .unwrap()
...
impl AchievementSetsDisplayRef {
    ...
    pub fn iter() -> impl Iterator<Item = Self> {
        TABLE_AchievementSetsDisplay.iter().enumerate().map(|(i, _)| Self(i))
    }
    pub fn iter_with_refs() -> impl Iterator<Item = (Self, &'static AchievementSetsDisplayRow)> {
        TABLE_AchievementSetsDisplay.iter().enumerate().map(|(i, x)| (Self(i), x))
    }
}
