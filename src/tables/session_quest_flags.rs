...
pub static TABLE_SessionQuestFlags: LazyLock<Vec<SessionQuestFlagsRow>> = LazyLock::new(|| {
    let df = DAT_LOADER.write().unwrap().get_table("data/balance/sessionquestflags.datc64").unwrap().clone();
    df.rows_iter()
...
impl SessionQuestFlagsRef {
    ...
    pub fn iter() -> impl Iterator<Item = Self> {
        TABLE_SessionQuestFlags.iter().enumerate().map(|(i, _)| Self(i))
    }
    pub fn iter_with_refs() -> impl Iterator<Item = (Self, &'static SessionQuestFlagsRow)> {
        TABLE_SessionQuestFlags.iter().enumerate().map(|(i, x)| (Self(i), x))
    }
}
