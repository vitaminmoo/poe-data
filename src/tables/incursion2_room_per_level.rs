...
pub static TABLE_Incursion2RoomPerLevel: LazyLock<Vec<Incursion2RoomPerLevelRow>> = LazyLock::new(|| {
    let df = DAT_LOADER
        .write()
        .unwrap()
...
impl Incursion2RoomPerLevelRef {
    ...
    pub fn iter() -> impl Iterator<Item = Self> {
        TABLE_Incursion2RoomPerLevel.iter().enumerate().map(|(i, _)| Self(i))
    }
    pub fn iter_with_refs() -> impl Iterator<Item = (Self, &'static Incursion2RoomPerLevelRow)> {
        TABLE_Incursion2RoomPerLevel.iter().enumerate().map(|(i, x)| (Self(i), x))
    }
}
