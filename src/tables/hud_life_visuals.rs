...
pub static TABLE_HudLifeVisuals: LazyLock<Vec<HudLifeVisualsRow>> = LazyLock::new(|| {
    let df = DAT_LOADER.write().unwrap().get_table("data/balance/hudlifevisuals.datc64").unwrap().clone();
    df.rows_iter()
...
impl HudLifeVisualsRef {
    ...
    pub fn iter() -> impl Iterator<Item = Self> {
        TABLE_HudLifeVisuals.iter().enumerate().map(|(i, _)| Self(i))
    }
    pub fn iter_with_refs() -> impl Iterator<Item = (Self, &'static HudLifeVisualsRow)> {
        TABLE_HudLifeVisuals.iter().enumerate().map(|(i, x)| (Self(i), x))
    }
}
