...
pub static TABLE_SentinelPassiveStats: LazyLock<Vec<SentinelPassiveStatsRow>> = LazyLock::new(|| {
    let df = DAT_LOADER
        .write()
        .unwrap()
...
impl SentinelPassiveStatsRef {
    ...
    pub fn iter() -> impl Iterator<Item = Self> {
        TABLE_SentinelPassiveStats.iter().enumerate().map(|(i, _)| Self(i))
    }
    pub fn iter_with_refs() -> impl Iterator<Item = (Self, &'static SentinelPassiveStatsRow)> {
        TABLE_SentinelPassiveStats.iter().enumerate().map(|(i, x)| (Self(i), x))
    }
}
