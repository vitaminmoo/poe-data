...
pub static TABLE_LegacyAtlasInfluenceOutcomes: LazyLock<Vec<LegacyAtlasInfluenceOutcomesRow>> = LazyLock::new(|| {
    let df = DAT_LOADER
        .write()
        .unwrap()
...
impl LegacyAtlasInfluenceOutcomesRef {
    ...
    pub fn iter() -> impl Iterator<Item = Self> {
        TABLE_LegacyAtlasInfluenceOutcomes.iter().enumerate().map(|(i, _)| Self(i))
    }
    pub fn iter_with_refs() -> impl Iterator<Item = (Self, &'static LegacyAtlasInfluenceOutcomesRow)> {
        TABLE_LegacyAtlasInfluenceOutcomes.iter().enumerate().map(|(i, x)| (Self(i), x))
    }
}
