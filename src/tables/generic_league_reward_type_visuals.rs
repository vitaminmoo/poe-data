...
pub static TABLE_GenericLeagueRewardTypeVisuals: LazyLock<Vec<GenericLeagueRewardTypeVisualsRow>> = LazyLock::new(|| {
    let df = DAT_LOADER
        .write()
        .unwrap()
...
impl GenericLeagueRewardTypeVisualsRef {
    ...
    pub fn iter() -> impl Iterator<Item = Self> {
        TABLE_GenericLeagueRewardTypeVisuals.iter().enumerate().map(|(i, _)| Self(i))
    }
    pub fn iter_with_refs() -> impl Iterator<Item = (Self, &'static GenericLeagueRewardTypeVisualsRow)> {
        TABLE_GenericLeagueRewardTypeVisuals.iter().enumerate().map(|(i, x)| (Self(i), x))
    }
}
