...
pub static TABLE_BuffVisualOrbArt: LazyLock<Vec<BuffVisualOrbArtRow>> = LazyLock::new(|| {
    let df = DAT_LOADER.write().unwrap().get_table("data/balance/buffvisualorbart.datc64").unwrap().clone();
    df.rows_iter()
...
impl BuffVisualOrbArtRef {
    ...
    pub fn iter() -> impl Iterator<Item = Self> {
        TABLE_BuffVisualOrbArt.iter().enumerate().map(|(i, _)| Self(i))
    }
    pub fn iter_with_refs() -> impl Iterator<Item = (Self, &'static BuffVisualOrbArtRow)> {
        TABLE_BuffVisualOrbArt.iter().enumerate().map(|(i, x)| (Self(i), x))
    }
}
