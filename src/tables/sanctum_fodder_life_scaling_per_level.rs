...
pub static TABLE_SanctumFodderLifeScalingPerLevel: LazyLock<Vec<SanctumFodderLifeScalingPerLevelRow>> = LazyLock::new(|| {
    let df = DAT_LOADER
        .write()
        .unwrap()
...
impl SanctumFodderLifeScalingPerLevelRef {
    ...
    pub fn iter() -> impl Iterator<Item = Self> {
        TABLE_SanctumFodderLifeScalingPerLevel.iter().enumerate().map(|(i, _)| Self(i))
    }
    pub fn iter_with_refs() -> impl Iterator<Item = (Self, &'static SanctumFodderLifeScalingPerLevelRow)> {
        TABLE_SanctumFodderLifeScalingPerLevel.iter().enumerate().map(|(i, x)| (Self(i), x))
    }
}
