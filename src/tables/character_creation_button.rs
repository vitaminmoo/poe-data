...
pub static TABLE_CharacterCreationButton: LazyLock<Vec<CharacterCreationButtonRow>> = LazyLock::new(|| {
    let df = DAT_LOADER
        .write()
        .unwrap()
...
impl CharacterCreationButtonRef {
    ...
    pub fn iter() -> impl Iterator<Item = Self> {
        TABLE_CharacterCreationButton.iter().enumerate().map(|(i, _)| Self(i))
    }
    pub fn iter_with_refs() -> impl Iterator<Item = (Self, &'static CharacterCreationButtonRow)> {
        TABLE_CharacterCreationButton.iter().enumerate().map(|(i, x)| (Self(i), x))
    }
}
