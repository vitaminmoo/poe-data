...
pub static TABLE_SupportGems: LazyLock<Vec<SupportGemsRow>> = LazyLock::new(|| {
    let df = DAT_LOADER.write().unwrap().get_table("data/balance/supportgems.datc64").unwrap().clone();
    df.rows_iter()
        .map(|row| SupportGemsRow {
            ...
            r#family: {
                ...
                let values = df
                    ...
                    .collect::<Vec<i64>>();
                values.into_iter().map(|value| SupportGemFamilyRef::new(value as usize)).collect()
            },
            r#is_lineage: {
...
impl SupportGemsRef {
    ...
    pub fn iter_with_refs() -> impl Iterator<Item = (Self, &'static SupportGemsRow)> {
        TABLE_SupportGems.iter().enumerate().map(|(i, x)| (Self(i), x))
    }
}
