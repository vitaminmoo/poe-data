...
pub static TABLE_RitualBalancePerLevel: LazyLock<Vec<RitualBalancePerLevelRow>> = LazyLock::new(|| {
    let df = DAT_LOADER
        .write()
        .unwrap()
...
impl RitualBalancePerLevelRef {
    ...
    pub fn iter() -> impl Iterator<Item = Self> {
        TABLE_RitualBalancePerLevel.iter().enumerate().map(|(i, _)| Self(i))
    }
    pub fn iter_with_refs() -> impl Iterator<Item = (Self, &'static RitualBalancePerLevelRow)> {
        TABLE_RitualBalancePerLevel.iter().enumerate().map(|(i, x)| (Self(i), x))
    }
}
