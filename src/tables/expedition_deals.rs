...
pub static TABLE_ExpeditionDeals: LazyLock<Vec<ExpeditionDealsRow>> = LazyLock::new(|| {
    let df = DAT_LOADER.write().unwrap().get_table("data/balance/expeditiondeals.datc64").unwrap().clone();
    df.rows_iter()
...
impl ExpeditionDealsRef {
    ...
    pub fn iter() -> impl Iterator<Item = Self> {
        TABLE_ExpeditionDeals.iter().enumerate().map(|(i, _)| Self(i))
    }
    pub fn iter_with_refs() -> impl Iterator<Item = (Self, &'static ExpeditionDealsRow)> {
        TABLE_ExpeditionDeals.iter().enumerate().map(|(i, x)| (Self(i), x))
    }
}
