...
pub static TABLE_TerrainPlugins: LazyLock<Vec<TerrainPluginsRow>> = LazyLock::new(|| {
    let df = DAT_LOADER.write().unwrap().get_table("data/balance/terrainplugins.datc64").unwrap().clone();
    df.rows_iter()
...
impl TerrainPluginsRef {
    ...
    pub fn iter() -> impl Iterator<Item = Self> {
        TABLE_TerrainPlugins.iter().enumerate().map(|(i, _)| Self(i))
    }
    pub fn iter_with_refs() -> impl Iterator<Item = (Self, &'static TerrainPluginsRow)> {
        TABLE_TerrainPlugins.iter().enumerate().map(|(i, x)| (Self(i), x))
    }
}
