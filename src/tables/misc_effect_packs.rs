...
pub static TABLE_MiscEffectPacks: LazyLock<Vec<MiscEffectPacksRow>> = LazyLock::new(|| {
    let df = DAT_LOADER.write().unwrap().get_table("data/balance/misceffectpacks.datc64").unwrap().clone();
    df.rows_iter()
...
impl MiscEffectPacksRef {
    ...
    pub fn iter() -> impl Iterator<Item = Self> {
        TABLE_MiscEffectPacks.iter().enumerate().map(|(i, _)| Self(i))
    }
    pub fn iter_with_refs() -> impl Iterator<Item = (Self, &'static MiscEffectPacksRow)> {
        TABLE_MiscEffectPacks.iter().enumerate().map(|(i, x)| (Self(i), x))
    }
}
