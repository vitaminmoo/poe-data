...
pub static TABLE_MonsterCategories: LazyLock<Vec<MonsterCategoriesRow>> = LazyLock::new(|| {
    let df = DAT_LOADER.write().unwrap().get_table("data/balance/monstercategories.datc64").unwrap().clone();
    df.rows_iter()
...
impl MonsterCategoriesRef {
    ...
    pub fn iter() -> impl Iterator<Item = Self> {
        TABLE_MonsterCategories.iter().enumerate().map(|(i, _)| Self(i))
    }
    pub fn iter_with_refs() -> impl Iterator<Item = (Self, &'static MonsterCategoriesRow)> {
        TABLE_MonsterCategories.iter().enumerate().map(|(i, x)| (Self(i), x))
    }
}
