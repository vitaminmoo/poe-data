...
pub static TABLE_ExpeditionRelics: LazyLock<Vec<ExpeditionRelicsRow>> = LazyLock::new(|| {
    let df = DAT_LOADER.write().unwrap().get_table("data/balance/expeditionrelics.datc64").unwrap().clone();
    df.rows_iter()
...
impl ExpeditionRelicsRef {
    ...
    pub fn iter() -> impl Iterator<Item = Self> {
        TABLE_ExpeditionRelics.iter().enumerate().map(|(i, _)| Self(i))
    }
    pub fn iter_with_refs() -> impl Iterator<Item = (Self, &'static ExpeditionRelicsRow)> {
        TABLE_ExpeditionRelics.iter().enumerate().map(|(i, x)| (Self(i), x))
    }
}
