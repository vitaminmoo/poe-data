...
pub static TABLE_NPCDialogueStyles: LazyLock<Vec<NPCDialogueStylesRow>> = LazyLock::new(|| {
    let df = DAT_LOADER.write().unwrap().get_table("data/balance/npcdialoguestyles.datc64").unwrap().clone();
    df.rows_iter()
...
impl NPCDialogueStylesRef {
    ...
    pub fn iter() -> impl Iterator<Item = Self> {
        TABLE_NPCDialogueStyles.iter().enumerate().map(|(i, _)| Self(i))
    }
    pub fn iter_with_refs() -> impl Iterator<Item = (Self, &'static NPCDialogueStylesRow)> {
        TABLE_NPCDialogueStyles.iter().enumerate().map(|(i, x)| (Self(i), x))
    }
}
