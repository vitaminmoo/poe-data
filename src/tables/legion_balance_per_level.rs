...
pub static TABLE_LegionBalancePerLevel: LazyLock<Vec<LegionBalancePerLevelRow>> = LazyLock::new(|| {
    let df = DAT_LOADER
        .write()
        .unwrap()
...
impl LegionBalancePerLevelRef {
    ...
    pub fn iter() -> impl Iterator<Item = Self> {
        TABLE_LegionBalancePerLevel.iter().enumerate().map(|(i, _)| Self(i))
    }
    pub fn iter_with_refs() -> impl Iterator<Item = (Self, &'static LegionBalancePerLevelRow)> {
        TABLE_LegionBalancePerLevel.iter().enumerate().map(|(i, x)| (Self(i), x))
    }
}
