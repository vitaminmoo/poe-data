...
pub static TABLE_PassiveJewelRadiiArt: LazyLock<Vec<PassiveJewelRadiiArtRow>> = LazyLock::new(|| {
    let df = DAT_LOADER
        .write()
        .unwrap()
...
impl PassiveJewelRadiiArtRef {
    ...
    pub fn iter() -> impl Iterator<Item = Self> {
        TABLE_PassiveJewelRadiiArt.iter().enumerate().map(|(i, _)| Self(i))
    }
    pub fn iter_with_refs() -> impl Iterator<Item = (Self, &'static PassiveJewelRadiiArtRow)> {
        TABLE_PassiveJewelRadiiArt.iter().enumerate().map(|(i, x)| (Self(i), x))
    }
}
