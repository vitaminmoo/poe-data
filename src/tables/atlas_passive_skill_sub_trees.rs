...
pub static TABLE_AtlasPassiveSkillSubTrees: LazyLock<Vec<AtlasPassiveSkillSubTreesRow>> = LazyLock::new(|| {
    let df = DAT_LOADER
        .write()
        .unwrap()
...
impl AtlasPassiveSkillSubTreesRef {
    ...
    pub fn iter() -> impl Iterator<Item = Self> {
        TABLE_AtlasPassiveSkillSubTrees.iter().enumerate().map(|(i, _)| Self(i))
    }
    pub fn iter_with_refs() -> impl Iterator<Item = (Self, &'static AtlasPassiveSkillSubTreesRow)> {
        TABLE_AtlasPassiveSkillSubTrees.iter().enumerate().map(|(i, x)| (Self(i), x))
    }
}
