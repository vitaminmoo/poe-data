...
pub static TABLE_BetrayalTargets: LazyLock<Vec<BetrayalTargetsRow>> = LazyLock::new(|| {
    let df = DAT_LOADER.write().unwrap().get_table("data/balance/betrayaltargets.datc64").unwrap().clone();
    df.rows_iter()
...
impl BetrayalTargetsRef {
    ...
    pub fn iter() -> impl Iterator<Item = Self> {
        TABLE_BetrayalTargets.iter().enumerate().map(|(i, _)| Self(i))
    }
    pub fn iter_with_refs() -> impl Iterator<Item = (Self, &'static BetrayalTargetsRow)> {
        TABLE_BetrayalTargets.iter().enumerate().map(|(i, x)| (Self(i), x))
    }
}
