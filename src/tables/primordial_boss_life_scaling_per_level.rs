...
pub static TABLE_PrimordialBossLifeScalingPerLevel: LazyLock<
    Vec<PrimordialBossLifeScalingPerLevelRow>,
> = LazyLock::new(|| {
    let df = DAT_LOADER
        .write()
        .unwrap()
...
impl PrimordialBossLifeScalingPerLevelRef {
    ...
    pub fn iter() -> impl Iterator<Item = Self> {
        TABLE_PrimordialBossLifeScalingPerLevel
            .iter()
            .enumerate()
            .map(|(i, _)| Self(i))
    }
    pub fn iter_with_refs(
    ) -> impl Iterator<Item = (Self, &'static PrimordialBossLifeScalingPerLevelRow)> {
        TABLE_PrimordialBossLifeScalingPerLevel
            .iter()
            .enumerate()
            .map(|(i, x)| (Self(i), x))
