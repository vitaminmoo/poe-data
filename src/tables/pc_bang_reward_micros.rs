...
pub static TABLE_PCBangRewardMicros: LazyLock<Vec<PCBangRewardMicrosRow>> = LazyLock::new(|| {
    let df = DAT_LOADER.write().unwrap().get_table("data/balance/pcbangrewardmicros.datc64").unwrap().clone();
    df.rows_iter()
...
impl PCBangRewardMicrosRef {
    ...
    pub fn iter() -> impl Iterator<Item = Self> {
        TABLE_PCBangRewardMicros.iter().enumerate().map(|(i, _)| Self(i))
    }
    pub fn iter_with_refs() -> impl Iterator<Item = (Self, &'static PCBangRewardMicrosRow)> {
        TABLE_PCBangRewardMicros.iter().enumerate().map(|(i, x)| (Self(i), x))
    }
}
