...
pub static TABLE_EndgameCorruptionMods: LazyLock<Vec<EndgameCorruptionModsRow>> = LazyLock::new(|| {
    let df = DAT_LOADER
        .write()
        .unwrap()
...
impl EndgameCorruptionModsRef {
    ...
    pub fn iter() -> impl Iterator<Item = Self> {
        TABLE_EndgameCorruptionMods.iter().enumerate().map(|(i, _)| Self(i))
    }
    pub fn iter_with_refs() -> impl Iterator<Item = (Self, &'static EndgameCorruptionModsRow)> {
        TABLE_EndgameCorruptionMods.iter().enumerate().map(|(i, x)| (Self(i), x))
    }
}
