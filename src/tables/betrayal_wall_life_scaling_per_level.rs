...
pub static TABLE_BetrayalWallLifeScalingPerLevel: LazyLock<Vec<BetrayalWallLifeScalingPerLevelRow>> = LazyLock::new(|| {
    let df = DAT_LOADER
        .write()
        .unwrap()
...
impl BetrayalWallLifeScalingPerLevelRef {
    ...
    pub fn iter() -> impl Iterator<Item = Self> {
        TABLE_BetrayalWallLifeScalingPerLevel.iter().enumerate().map(|(i, _)| Self(i))
    }
    pub fn iter_with_refs() -> impl Iterator<Item = (Self, &'static BetrayalWallLifeScalingPerLevelRow)> {
        TABLE_BetrayalWallLifeScalingPerLevel.iter().enumerate().map(|(i, x)| (Self(i), x))
    }
}
