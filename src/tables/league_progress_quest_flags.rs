...
pub static TABLE_LeagueProgressQuestFlags: LazyLock<Vec<LeagueProgressQuestFlagsRow>> = LazyLock::new(|| {
    let df = DAT_LOADER
        .write()
        .unwrap()
...
impl LeagueProgressQuestFlagsRef {
    ...
    pub fn iter() -> impl Iterator<Item = Self> {
        TABLE_LeagueProgressQuestFlags.iter().enumerate().map(|(i, _)| Self(i))
    }
    pub fn iter_with_refs() -> impl Iterator<Item = (Self, &'static LeagueProgressQuestFlagsRow)> {
        TABLE_LeagueProgressQuestFlags.iter().enumerate().map(|(i, x)| (Self(i), x))
    }
}
