...
pub static TABLE_GrantedEffectStatSetsPerLevel: LazyLock<Vec<GrantedEffectStatSetsPerLevelRow>> =
    LazyLock::new(|| {
        let df = DAT_LOADER
            .write()
            .unwrap()
        ...
        df.rows_iter()
            .map(|row| GrantedEffectStatSetsPerLevelRow {
                ...
                r#stat_interpolations: {
                    ...
                    values
                        .into_iter()
                        .map(|value| StatInterpolationTypes::from_repr(value as usize))
                        .collect()
                },
...
impl GrantedEffectStatSetsPerLevelRef {
    ...
    pub fn iter() -> impl Iterator<Item = Self> {
        TABLE_GrantedEffectStatSetsPerLevel
            .iter()
            .enumerate()
            .map(|(i, _)| Self(i))
    }
    pub fn iter_with_refs(
    ) -> impl Iterator<Item = (Self, &'static GrantedEffectStatSetsPerLevelRow)> {
        TABLE_GrantedEffectStatSetsPerLevel
            .iter()
            .enumerate()
            .map(|(i, x)| (Self(i), x))
