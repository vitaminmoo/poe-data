...
pub static TABLE_GraphicalItemReceptacle: LazyLock<Vec<GraphicalItemReceptacleRow>> = LazyLock::new(|| {
    let df = DAT_LOADER
        .write()
        .unwrap()
...
impl GraphicalItemReceptacleRef {
    ...
    pub fn iter() -> impl Iterator<Item = Self> {
        TABLE_GraphicalItemReceptacle.iter().enumerate().map(|(i, _)| Self(i))
    }
    pub fn iter_with_refs() -> impl Iterator<Item = (Self, &'static GraphicalItemReceptacleRow)> {
        TABLE_GraphicalItemReceptacle.iter().enumerate().map(|(i, x)| (Self(i), x))
    }
}
