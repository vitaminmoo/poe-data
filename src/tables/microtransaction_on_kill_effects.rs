...
pub static TABLE_MicrotransactionOnKillEffects: LazyLock<Vec<MicrotransactionOnKillEffectsRow>> =
    LazyLock::new(|| {
        let df = DAT_LOADER
            .write()
            .unwrap()
...
impl MicrotransactionOnKillEffectsRef {
    ...
    pub fn iter() -> impl Iterator<Item = Self> {
        TABLE_MicrotransactionOnKillEffects
            .iter()
            .enumerate()
            .map(|(i, _)| Self(i))
    }
    pub fn iter_with_refs(
    ) -> impl Iterator<Item = (Self, &'static MicrotransactionOnKillEffectsRow)> {
        TABLE_MicrotransactionOnKillEffects
            .iter()
            .enumerate()
            .map(|(i, x)| (Self(i), x))
