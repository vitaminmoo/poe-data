...
pub static TABLE_MonsterHeightBrackets: LazyLock<Vec<MonsterHeightBracketsRow>> = LazyLock::new(|| {
    let df = DAT_LOADER
        .write()
        .unwrap()
...
impl MonsterHeightBracketsRef {
    ...
    pub fn iter() -> impl Iterator<Item = Self> {
        TABLE_MonsterHeightBrackets.iter().enumerate().map(|(i, _)| Self(i))
    }
    pub fn iter_with_refs() -> impl Iterator<Item = (Self, &'static MonsterHeightBracketsRow)> {
        TABLE_MonsterHeightBrackets.iter().enumerate().map(|(i, x)| (Self(i), x))
    }
}
