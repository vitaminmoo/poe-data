...
pub static TABLE_BattlePassTracks: LazyLock<Vec<BattlePassTracksRow>> = LazyLock::new(|| {
    let df = DAT_LOADER.write().unwrap().get_table("data/balance/battlepasstracks.datc64").unwrap().clone();
    df.rows_iter()
...
impl BattlePassTracksRef {
    ...
    pub fn iter() -> impl Iterator<Item = Self> {
        TABLE_BattlePassTracks.iter().enumerate().map(|(i, _)| Self(i))
    }
    pub fn iter_with_refs() -> impl Iterator<Item = (Self, &'static BattlePassTracksRow)> {
        TABLE_BattlePassTracks.iter().enumerate().map(|(i, x)| (Self(i), x))
    }
}
