...
pub static TABLE_ShapeShiftFormClones: LazyLock<Vec<ShapeShiftFormClonesRow>> = LazyLock::new(|| {
    let df = DAT_LOADER
        .write()
        .unwrap()
...
impl ShapeShiftFormClonesRef {
    ...
    pub fn iter() -> impl Iterator<Item = Self> {
        TABLE_ShapeShiftFormClones.iter().enumerate().map(|(i, _)| Self(i))
    }
    pub fn iter_with_refs() -> impl Iterator<Item = (Self, &'static ShapeShiftFormClonesRow)> {
        TABLE_ShapeShiftFormClones.iter().enumerate().map(|(i, x)| (Self(i), x))
    }
}
