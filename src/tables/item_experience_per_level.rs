...
pub static TABLE_ItemExperiencePerLevel: LazyLock<Vec<ItemExperiencePerLevelRow>> = LazyLock::new(|| {
    let df = DAT_LOADER
        .write()
        .unwrap()
...
impl ItemExperiencePerLevelRef {
    ...
    pub fn iter() -> impl Iterator<Item = Self> {
        TABLE_ItemExperiencePerLevel.iter().enumerate().map(|(i, _)| Self(i))
    }
    pub fn iter_with_refs() -> impl Iterator<Item = (Self, &'static ItemExperiencePerLevelRow)> {
        TABLE_ItemExperiencePerLevel.iter().enumerate().map(|(i, x)| (Self(i), x))
    }
}
