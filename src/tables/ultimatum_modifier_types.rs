...
pub static TABLE_UltimatumModifierTypes: LazyLock<Vec<UltimatumModifierTypesRow>> = LazyLock::new(|| {
    let df = DAT_LOADER
        .write()
        .unwrap()
...
impl UltimatumModifierTypesRef {
    ...
    pub fn iter() -> impl Iterator<Item = Self> {
        TABLE_UltimatumModifierTypes.iter().enumerate().map(|(i, _)| Self(i))
    }
    pub fn iter_with_refs() -> impl Iterator<Item = (Self, &'static UltimatumModifierTypesRow)> {
        TABLE_UltimatumModifierTypes.iter().enumerate().map(|(i, x)| (Self(i), x))
    }
}
