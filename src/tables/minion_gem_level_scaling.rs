...
pub static TABLE_MinionGemLevelScaling: LazyLock<Vec<MinionGemLevelScalingRow>> = LazyLock::new(|| {
    let df = DAT_LOADER
        .write()
        .unwrap()
...
impl MinionGemLevelScalingRef {
    ...
    pub fn iter() -> impl Iterator<Item = Self> {
        TABLE_MinionGemLevelScaling.iter().enumerate().map(|(i, _)| Self(i))
    }
    pub fn iter_with_refs() -> impl Iterator<Item = (Self, &'static MinionGemLevelScalingRow)> {
        TABLE_MinionGemLevelScaling.iter().enumerate().map(|(i, x)| (Self(i), x))
    }
}
