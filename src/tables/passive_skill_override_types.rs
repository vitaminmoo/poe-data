...
pub static TABLE_PassiveSkillOverrideTypes: LazyLock<Vec<PassiveSkillOverrideTypesRow>> = LazyLock::new(|| {
    let df = DAT_LOADER
        .write()
        .unwrap()
...
impl PassiveSkillOverrideTypesRef {
    ...
    pub fn iter() -> impl Iterator<Item = Self> {
        TABLE_PassiveSkillOverrideTypes.iter().enumerate().map(|(i, _)| Self(i))
    }
    pub fn iter_with_refs() -> impl Iterator<Item = (Self, &'static PassiveSkillOverrideTypesRow)> {
        TABLE_PassiveSkillOverrideTypes.iter().enumerate().map(|(i, x)| (Self(i), x))
    }
}
