...
pub static TABLE_MavenJewelRadiusKeystones: LazyLock<Vec<MavenJewelRadiusKeystonesRow>> = LazyLock::new(|| {
    let df = DAT_LOADER
        .write()
        .unwrap()
...
impl MavenJewelRadiusKeystonesRef {
    ...
    pub fn iter() -> impl Iterator<Item = Self> {
        TABLE_MavenJewelRadiusKeystones.iter().enumerate().map(|(i, _)| Self(i))
    }
    pub fn iter_with_refs() -> impl Iterator<Item = (Self, &'static MavenJewelRadiusKeystonesRow)> {
        TABLE_MavenJewelRadiusKeystones.iter().enumerate().map(|(i, x)| (Self(i), x))
    }
}
