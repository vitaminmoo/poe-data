...
pub static TABLE_IncubatorBaseRequiredKillsPerLevel: LazyLock<
    Vec<IncubatorBaseRequiredKillsPerLevelRow>,
> = LazyLock::new(|| {
    let df = DAT_LOADER
        .write()
        .unwrap()
...
impl IncubatorBaseRequiredKillsPerLevelRef {
    ...
    pub fn iter() -> impl Iterator<Item = Self> {
        TABLE_IncubatorBaseRequiredKillsPerLevel
            .iter()
            .enumerate()
            .map(|(i, _)| Self(i))
    }
    pub fn iter_with_refs(
    ) -> impl Iterator<Item = (Self, &'static IncubatorBaseRequiredKillsPerLevelRow)> {
        TABLE_IncubatorBaseRequiredKillsPerLevel
            .iter()
            .enumerate()
            .map(|(i, x)| (Self(i), x))
