...
pub static TABLE_MicrotransactionCharacterPortraitVariations: LazyLock<Vec<MicrotransactionCharacterPortraitVariationsRow>> = LazyLock::new(|| {
    let df = DAT_LOADER
        .write()
        .unwrap()
...
impl MicrotransactionCharacterPortraitVariationsRef {
    ...
    pub fn iter() -> impl Iterator<Item = Self> {
        TABLE_MicrotransactionCharacterPortraitVariations.iter().enumerate().map(|(i, _)| Self(i))
    }
    pub fn iter_with_refs() -> impl Iterator<Item = (Self, &'static MicrotransactionCharacterPortraitVariationsRow)> {
        TABLE_MicrotransactionCharacterPortraitVariations.iter().enumerate().map(|(i, x)| (Self(i), x))
    }
}
