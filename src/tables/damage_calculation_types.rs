...
pub static TABLE_DamageCalculationTypes: LazyLock<Vec<DamageCalculationTypesRow>> = LazyLock::new(|| {
    let df = DAT_LOADER
        .write()
        .unwrap()
...
impl DamageCalculationTypesRef {
    ...
    pub fn iter() -> impl Iterator<Item = Self> {
        TABLE_DamageCalculationTypes.iter().enumerate().map(|(i, _)| Self(i))
    }
    pub fn iter_with_refs() -> impl Iterator<Item = (Self, &'static DamageCalculationTypesRow)> {
        TABLE_DamageCalculationTypes.iter().enumerate().map(|(i, x)| (Self(i), x))
    }
}
