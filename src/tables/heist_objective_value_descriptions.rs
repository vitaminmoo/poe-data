...
pub static TABLE_HeistObjectiveValueDescriptions: LazyLock<
    Vec<HeistObjectiveValueDescriptionsRow>,
> = LazyLock::new(|| {
    let df = DAT_LOADER
        .write()
        .unwrap()
...
impl HeistObjectiveValueDescriptionsRef {
    ...
    pub fn iter() -> impl Iterator<Item = Self> {
        TABLE_HeistObjectiveValueDescriptions
            .iter()
            .enumerate()
            .map(|(i, _)| Self(i))
    }
    pub fn iter_with_refs(
    ) -> impl Iterator<Item = (Self, &'static HeistObjectiveValueDescriptionsRow)> {
        TABLE_HeistObjectiveValueDescriptions
            .iter()
            .enumerate()
            .map(|(i, x)| (Self(i), x))
