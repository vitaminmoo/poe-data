...
pub static TABLE_AtlasUpgradesInventoryLayout: LazyLock<Vec<AtlasUpgradesInventoryLayoutRow>> = LazyLock::new(|| {
    let df = DAT_LOADER
        .write()
        .unwrap()
...
impl AtlasUpgradesInventoryLayoutRef {
    ...
    pub fn iter() -> impl Iterator<Item = Self> {
        TABLE_AtlasUpgradesInventoryLayout.iter().enumerate().map(|(i, _)| Self(i))
    }
    pub fn iter_with_refs() -> impl Iterator<Item = (Self, &'static AtlasUpgradesInventoryLayoutRow)> {
        TABLE_AtlasUpgradesInventoryLayout.iter().enumerate().map(|(i, x)| (Self(i), x))
    }
}
