...
pub static TABLE_MicrotransactionPortalVariations: LazyLock<
    Vec<MicrotransactionPortalVariationsRow>,
> = LazyLock::new(|| {
    let df = DAT_LOADER
        .write()
        .unwrap()
...
impl MicrotransactionPortalVariationsRef {
    ...
    pub fn iter() -> impl Iterator<Item = Self> {
        TABLE_MicrotransactionPortalVariations
            .iter()
            .enumerate()
            .map(|(i, _)| Self(i))
    }
    pub fn iter_with_refs(
    ) -> impl Iterator<Item = (Self, &'static MicrotransactionPortalVariationsRow)> {
        TABLE_MicrotransactionPortalVariations
            .iter()
            .enumerate()
            .map(|(i, x)| (Self(i), x))
