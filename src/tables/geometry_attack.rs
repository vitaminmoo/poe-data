...
pub static TABLE_GeometryAttack: LazyLock<Vec<GeometryAttackRow>> = LazyLock::new(|| {
    let df = DAT_LOADER.write().unwrap().get_table("data/balance/geometryattack.datc64").unwrap().clone();
    df.rows_iter()
...
impl GeometryAttackRef {
    ...
    pub fn iter() -> impl Iterator<Item = Self> {
        TABLE_GeometryAttack.iter().enumerate().map(|(i, _)| Self(i))
    }
    pub fn iter_with_refs() -> impl Iterator<Item = (Self, &'static GeometryAttackRow)> {
        TABLE_GeometryAttack.iter().enumerate().map(|(i, x)| (Self(i), x))
    }
}
