...
pub static TABLE_UltimatumTrialMasterAudio: LazyLock<Vec<UltimatumTrialMasterAudioRow>> = LazyLock::new(|| {
    let df = DAT_LOADER
        .write()
        .unwrap()
...
impl UltimatumTrialMasterAudioRef {
    ...
    pub fn iter() -> impl Iterator<Item = Self> {
        TABLE_UltimatumTrialMasterAudio.iter().enumerate().map(|(i, _)| Self(i))
    }
    pub fn iter_with_refs() -> impl Iterator<Item = (Self, &'static UltimatumTrialMasterAudioRow)> {
        TABLE_UltimatumTrialMasterAudio.iter().enumerate().map(|(i, x)| (Self(i), x))
    }
}
