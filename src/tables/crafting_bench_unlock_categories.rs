...
pub static TABLE_CraftingBenchUnlockCategories: LazyLock<Vec<CraftingBenchUnlockCategoriesRow>> = LazyLock::new(|| {
    let df = DAT_LOADER
        .write()
        .unwrap()
...
impl CraftingBenchUnlockCategoriesRef {
    ...
    pub fn iter() -> impl Iterator<Item = Self> {
        TABLE_CraftingBenchUnlockCategories.iter().enumerate().map(|(i, _)| Self(i))
    }
    pub fn iter_with_refs() -> impl Iterator<Item = (Self, &'static CraftingBenchUnlockCategoriesRow)> {
        TABLE_CraftingBenchUnlockCategories.iter().enumerate().map(|(i, x)| (Self(i), x))
    }
}
