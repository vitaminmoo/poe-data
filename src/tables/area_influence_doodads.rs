...
pub static TABLE_AreaInfluenceDoodads: LazyLock<Vec<AreaInfluenceDoodadsRow>> = LazyLock::new(|| {
    let df = DAT_LOADER
        .write()
        .unwrap()
...
impl AreaInfluenceDoodadsRef {
    ...
    pub fn iter() -> impl Iterator<Item = Self> {
        TABLE_AreaInfluenceDoodads.iter().enumerate().map(|(i, _)| Self(i))
    }
    pub fn iter_with_refs() -> impl Iterator<Item = (Self, &'static AreaInfluenceDoodadsRow)> {
        TABLE_AreaInfluenceDoodads.iter().enumerate().map(|(i, x)| (Self(i), x))
    }
}
