...
pub static TABLE_DelveLevelScaling: LazyLock<Vec<DelveLevelScalingRow>> = LazyLock::new(|| {
    let df = DAT_LOADER.write().unwrap().get_table("data/balance/delvelevelscaling.datc64").unwrap().clone();
    df.rows_iter()
...
impl DelveLevelScalingRef {
    ...
    pub fn iter() -> impl Iterator<Item = Self> {
        TABLE_DelveLevelScaling.iter().enumerate().map(|(i, _)| Self(i))
    }
    pub fn iter_with_refs() -> impl Iterator<Item = (Self, &'static DelveLevelScalingRow)> {
        TABLE_DelveLevelScaling.iter().enumerate().map(|(i, x)| (Self(i), x))
    }
}
