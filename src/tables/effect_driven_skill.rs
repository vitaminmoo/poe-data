...
pub static TABLE_EffectDrivenSkill: LazyLock<Vec<EffectDrivenSkillRow>> = LazyLock::new(|| {
    let df = DAT_LOADER.write().unwrap().get_table("data/balance/effectdrivenskill.datc64").unwrap().clone();
    df.rows_iter()
...
impl EffectDrivenSkillRef {
    ...
    pub fn iter() -> impl Iterator<Item = Self> {
        TABLE_EffectDrivenSkill.iter().enumerate().map(|(i, _)| Self(i))
    }
    pub fn iter_with_refs() -> impl Iterator<Item = (Self, &'static EffectDrivenSkillRow)> {
        TABLE_EffectDrivenSkill.iter().enumerate().map(|(i, x)| (Self(i), x))
    }
}
