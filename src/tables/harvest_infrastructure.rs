...
pub static TABLE_HarvestInfrastructure: LazyLock<Vec<HarvestInfrastructureRow>> = LazyLock::new(|| {
    let df = DAT_LOADER
        .write()
        .unwrap()
...
impl HarvestInfrastructureRef {
    ...
    pub fn iter() -> impl Iterator<Item = Self> {
        TABLE_HarvestInfrastructure.iter().enumerate().map(|(i, _)| Self(i))
    }
    pub fn iter_with_refs() -> impl Iterator<Item = (Self, &'static HarvestInfrastructureRow)> {
        TABLE_HarvestInfrastructure.iter().enumerate().map(|(i, x)| (Self(i), x))
    }
}
