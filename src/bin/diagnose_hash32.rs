...
fn main() {
    ...
    let files: Vec<String> = dl.get_file_list().into_iter().filter(|f| f.to_lowercase().contains("miscanimated.datc64")).collect();
    ...
    for offset in 0..row_len.saturating_sub(3) {
        ...
        println!("{:3}    | {:6} | {:10} | {:.4}  | {:.4} | {:2}/16   | L:{:3} M:{:3} | Odd:{:.2} {:.2} | {}", offset, min_val, max_val, entropy, entropy_ratio, filled_buckets, lsb_set.len(), msb_set.len(), lsb_odd_pct, msb_odd_pct, marker);
    }
}
