...
fn main() {
    ...
    // Search for hideouts file
    let files: Vec<String> = dl.get_file_list().into_iter().filter(|f| f.to_lowercase().contains("hideouts.datc64")).collect();
    
    if files.is_empty() {
        eprintln!("Could not find Hideouts table");
        return;
    }
    ...
    // Check every 2-byte window
    println!("Offset | MinVal | MaxVal | Entropy | Ratio  | Buckets | LSB   MSB   | Odd (L M)     | Note");
    println!("-------|--------|--------|---------|--------|---------|-------------|---------------|-----");
    for offset in 0..row_len.saturating_sub(1) {
        let mut values = Vec::new();
        for row in &rows {
            let val = row.slice(offset..offset+2).get_u16_le();
            values.push(val);
        }
        ...
        // Buckets of 4096 (16 buckets for u16)
        ...
        for &val in &values {
            buckets[(val / 4096) as usize] += 1;
            let lsb = (val & 0xFF) as u8;
            let msb = (val >> 8) as u8;
            lsb_set.insert(lsb);
            msb_set.insert(msb);
            
            if lsb % 2 != 0 { lsb_odd += 1; }
            if msb % 2 != 0 { msb_odd += 1; }
        }
        ...
        let is_target = offset == 16 || offset == 17 || offset == 41;
        let marker = if is_target { "***" } else { "   " };
        
        println!("{:3}    | {:5}  | {:5}  | {:.4}  | {:.4} | {:2}/16   | L:{:3} M:{:3} | Odd:{:.2} {:.2} | {}", offset, min_val, max_val, entropy, entropy_ratio, filled_buckets, lsb_set.len(), msb_set.len(), lsb_odd_pct, msb_odd_pct, marker);
    }
}